#!/usr/bin/env python3
"""
HFT CLI - Rust HFT × Agno 24/7 AI Trading Platform 命令行工具
========================================================

統一的命令行界面，用於管理代理工作流程和交易系統

使用方式:
  python hft_cli.py agent --list                    # 列出所有代理
  python hft_cli.py agent --start hft-ops           # 启动 HFT Operations Agent
  python hft_cli.py agent --start ml-workflow       # 启动 ML Workflow Agent
  python hft_cli.py workflow --run complete-ml      # 運行完整 ML 工作流程
  python hft_cli.py trading --start BTCUSDT         # 開始交易
  python hft_cli.py status --all                    # 查看系統狀態
"""
...
class HFTCLIManager:
    ...
    def _load_modules(self):
        ...
        try:
            ...
            # 載入 HFT Operations Agent
            from hft_operations_agent import HFTOperationsAgent, OperationMode
            self.agents['hft-ops'] = {
            ...
            # 載入 ML Workflow Agent  
            from ml_workflow_agent import MLWorkflowAgent, ExecutionMode
            self.agents['ml-workflow'] = {
            ...
            logger.info("✅ 成功載入所有代理模塊")
            
        except ImportError as e:
            logger.error(f"❌ 載入模塊失敗: {e}")
            raise HFTCLIError(f"無法載入必要模塊: {e}")
    ...
    async def start_agent(self, agent_id: str, mode: str = 'real') -> Dict[str, Any]:
        ...
        try:
            # 根據代理類型選擇模式
            if agent_id == 'hft-ops':
                mode_value = getattr(agent_info['mode_enum'], mode.upper())
            ...
            # 創建代理實例
            agent = agent_info['class'](mode=mode_value)
            
            # 根據代理類型執行相應操作
            if agent_id == 'hft-ops':
                result = await agent.run_operations_cycle()
                logger.info(f"📊 營運週期結果: {result.get('status')}")
                ...
                logger.info(f"📊 ML 工作流程結果: {result.get('status')}")
    ...
    async def _run_complete_ml_workflow(self, symbol: str = "BTCUSDT", **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            # 創建 ML 工作流程代理
            ml_agent = MLWorkflowAgent(mode=ExecutionMode.REAL)
    ...
    async def _run_hft_operations_workflow(self, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            # 創建 HFT 營運代理
            hft_agent = HFTOperationsAgent(mode=OperationMode.REAL)
    ...
    async def _run_coordination_workflow(self, symbol: str = "BTCUSDT", **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            training_result = await ml_agent.training_agent.train_tlob_model(symbol, training_config)
            ...
            if training_result["status"] == "success":
                ...
                deployment_result = await hft_agent.execution_agent.execute_trading_command(deployment_command)
    ...
    async def start_trading(self, symbol: str, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            # 使用 HFT Operations Agent 執行交易
            from hft_operations_agent import HFTOperationsAgent, OperationMode
            hft_agent = HFTOperationsAgent(mode=OperationMode.REAL)
            
            # 構建交易命令
            trading_command = {
                "type": "START_LIVE_TRADING",
                "symbol": symbol,
                "model_path": kwargs.get("model_path", f"/models/{symbol}_default.safetensors"),
                "capital": kwargs.get("capital", 1000.0)
            }
            
            result = await hft_agent.execution_agent.execute_trading_command(trading_command)
...
def create_parser():
    ...
    parser = argparse.ArgumentParser(
        description='HFT CLI - Rust HFT × Agno 24/7 AI Trading Platform',
