...
> **Integrated into Monday.** This workspace now lives at `products/ploy` in
> [`proerror77/monday`](https://github.com/proerror77/monday). Monday's
> `rust_hft` runtime remains the only production authority for risk, OMS,
> reconciliation, cancellation, and order execution. PLOY deployment workflows
> under this directory are retained as inactive historical contracts and must
> not be enabled without a separate secrets, environment, runner, and safety
> review. Live trading remains disabled. See [MIGRATION.md](MIGRATION.md).
