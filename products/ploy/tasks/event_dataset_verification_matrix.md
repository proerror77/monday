...
| --- | --- | --- |
| Research crate typecheck | `cargo check --manifest-path products/ploy/Cargo.toml --locked -p ploy-research --tests` | exits 0 |
| Research crate unit tests | `cargo test --manifest-path products/ploy/Cargo.toml --locked -p ploy-research --lib` | exits 0 |
| Dataset test discovery | `cargo test --manifest-path products/ploy/Cargo.toml --locked -p ploy-research --lib -- --list` | the expected dataset/split/manifest tests are present |
| Dataset-focused tests | `cargo test --manifest-path products/ploy/Cargo.toml --locked -p ploy-research dataset:: --lib -- --nocapture` | exits 0 |
| Conditional export example | `cargo test --manifest-path products/ploy/Cargo.toml --locked -p ploy-research --example factor_research --features db,polars-export -- --nocapture` | exits 0 when the change touches the export surface |
| Current product retirement contract | `cargo test --manifest-path products/ploy/Cargo.toml --locked -p ploy --test workspace_runtime_retirement` | exits 0 |
...
git diff --name-only <base>...HEAD -- \
  products/ploy/crates/ploy-research \
  products/ploy/crates/ploy-feed-loaders \
  products/ploy/crates/ploy-strategy-bundles \
  products/ploy/crates/ploy-strategy-runtime \
  products/ploy/apps/ploy-backtest \
  products/ploy/apps/ploy-replay \
  products/ploy/Cargo.toml \
