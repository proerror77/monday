...
st.set_page_config(
    page_title="HFT Controller",
    page_icon="🎯",
    layout="wide",
)
st.markdown(CUSTOM_CSS, unsafe_allow_html=True)
agent_name = "hft_controller"


async def header():
    st.markdown("<h1 class='heading'>🎯 HFT Controller</h1>", unsafe_allow_html=True)
    st.markdown(
        "<p class='subheading'>Central control agent for managing the entire HFT system infrastructure.</p>",
...
async def body() -> None:
    ...
    ####################################################################
    user_id = st.sidebar.text_input(":technologist: Username", value="HFT-Admin")
    ...
    if (
        ...
        logger.info("---*--- Creating HFT Controller Agent ---*---")
        hft_controller = get_hft_controller(user_id=user_id, model_id=model_id)
        st.session_state[agent_name]["agent"] = hft_controller
        st.session_state["selected_model"] = model_id
    else:
        hft_controller = st.session_state[agent_name]["agent"]

    ####################################################################
    # Load Agent Session from the database
    ####################################################################
    try:
        st.session_state[agent_name]["session_id"] = hft_controller.load_session()
    except Exception:
    ...
    ####################################################################
    if hft_controller.memory:
        agent_runs = hft_controller.memory.runs
    ...
    # Get user input
    ####################################################################
    if prompt := st.chat_input("🎯 How can I help manage the HFT system?"):
    ...
    if last_message and last_message.get("role") == "user":
        ...
        with st.chat_message("assistant"):
            ...
            with st.spinner(":thinking_face: Managing HFT system..."):
                response = ""
                try:
                    # Run the agent and stream the response
                    run_response = await hft_controller.arun(user_message, stream=True)
                    ...
                    if hft_controller.run_response is not None:
                        await add_message(agent_name, "assistant", response, hft_controller.run_response.tools)
                    else:
    ...
    # Session selector
    ####################################################################
    await session_selector(agent_name, hft_controller, get_hft_controller, user_id, model_id)

    ####################################################################
    # About section
    ####################################################################
    await utilities_widget(agent_name, hft_controller)
