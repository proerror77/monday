...
async def header():
    st.markdown("<h1 class='heading'>🧠 Model Trainer</h1>", unsafe_allow_html=True)
    st.markdown(
        "<p class='subheading'>Deep learning specialist for HFT model development, architecture design, and training workflows.</p>",
...
async def body() -> None:
    ...
    ####################################################################
    user_id = st.sidebar.text_input(":technologist: Username", value="HFT-ML")
    ...
    if (
        ...
        logger.info("---*--- Creating Model Trainer Agent ---*---")
        model_trainer = get_model_trainer(user_id=user_id, model_id=model_id)
        st.session_state[agent_name]["agent"] = model_trainer
        st.session_state["selected_model"] = model_id
    else:
        model_trainer = st.session_state[agent_name]["agent"]

    ####################################################################
    # Load Agent Session from the database
    ####################################################################
    try:
        st.session_state[agent_name]["session_id"] = model_trainer.load_session()
    except Exception:
    ...
    ####################################################################
    if model_trainer.memory:
        agent_runs = model_trainer.memory.runs
    ...
    # Get user input
    ####################################################################
    if prompt := st.chat_input("🧠 How can I help with model training?"):
    ...
    if last_message and last_message.get("role") == "user":
        ...
        with st.chat_message("assistant"):
            ...
            with st.spinner(":thinking_face: Training models..."):
                response = ""
                try:
                    # Run the agent and stream the response
                    run_response = await model_trainer.arun(user_message, stream=True)
                    ...
                    if model_trainer.run_response is not None:
                        await add_message(agent_name, "assistant", response, model_trainer.run_response.tools)
                    else:
    ...
    # Session selector
    ####################################################################
    await session_selector(agent_name, model_trainer, get_model_trainer, user_id, model_id)

    ####################################################################
    # About section
    ####################################################################
    await utilities_widget(agent_name, model_trainer)
