...
# -*- Dev database running on port 5433:5432 (ML workspace)
dev_db = PgVectorDb(
    name=f"{ws_settings.ws_name}-db",
    pg_user="ai",
    pg_password="ai",
    pg_database="ai",
    # Connect to this db on port 5433 (avoid conflict with master workspace)
    host_port=5433,
)
...
# -*- Streamlit running on port 8502:8501 (ML workspace)
...
# -*- FastApi running on port 8001:8000 (ML workspace)
dev_fastapi = FastApi(
