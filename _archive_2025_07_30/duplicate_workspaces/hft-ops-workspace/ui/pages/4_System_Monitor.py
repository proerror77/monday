...
st.set_page_config(
    page_title="Ops System Monitor",
    page_icon="📊",
    layout="wide",
)
st.markdown(CUSTOM_CSS, unsafe_allow_html=True)
agent_name = "ops_system_monitor"


async def header():
    st.markdown("<h1 class='heading'>📊 Ops System Monitor</h1>", unsafe_allow_html=True)
    st.markdown(
        "<p class='subheading'>Comprehensive system health monitoring and performance analysis for HFT infrastructure.</p>",
...
async def body() -> None:
    ...
    ####################################################################
    user_id = st.sidebar.text_input(":technologist: Username", value="HFT-Ops")
    ...
    if (
        ...
        logger.info("---*--- Creating Ops System Monitor Agent ---*---")
        system_monitor = get_system_monitor(user_id=user_id, model_id=model_id)
        st.session_state[agent_name]["agent"] = system_monitor
        st.session_state["selected_model"] = model_id
    else:
        system_monitor = st.session_state[agent_name]["agent"]

    ####################################################################
    # Load Agent Session from the database
    ####################################################################
    try:
        st.session_state[agent_name]["session_id"] = system_monitor.load_session()
    except Exception:
    ...
    ####################################################################
    if system_monitor.memory:
        agent_runs = system_monitor.memory.runs
    ...
    # Get user input
    ####################################################################
    if prompt := st.chat_input("📊 What would you like to monitor?"):
    ...
    if last_message and last_message.get("role") == "user":
        ...
        with st.chat_message("assistant"):
            ...
            with st.spinner(":thinking_face: Analyzing system metrics..."):
                response = ""
                try:
                    # Run the agent and stream the response
                    run_response = await system_monitor.arun(user_message, stream=True)
                    ...
                    if system_monitor.run_response is not None:
                        await add_message(agent_name, "assistant", response, system_monitor.run_response.tools)
                    else:
    ...
    # Session selector
    ####################################################################
    await session_selector(agent_name, system_monitor, get_system_monitor, user_id, model_id)

    ####################################################################
    # About section
    ####################################################################
    await utilities_widget(agent_name, system_monitor)
