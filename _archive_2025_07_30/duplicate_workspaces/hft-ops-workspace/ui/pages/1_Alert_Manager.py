...
async def header():
    st.markdown("<h1 class='heading'>⚠️ Alert Manager</h1>", unsafe_allow_html=True)
    st.markdown(
        "<p class='subheading'>Central alert processing and coordination system for all HFT operational alerts.</p>",
...
async def body() -> None:
    ...
    if (
        ...
        logger.info("---*--- Creating Alert Manager Agent ---*---")
        alert_manager = get_alert_manager(user_id=user_id, model_id=model_id)
        st.session_state[agent_name]["agent"] = alert_manager
        st.session_state["selected_model"] = model_id
    else:
        alert_manager = st.session_state[agent_name]["agent"]

    ####################################################################
    # Load Agent Session from the database
    ####################################################################
    try:
        st.session_state[agent_name]["session_id"] = alert_manager.load_session()
    except Exception:
    ...
    ####################################################################
    if alert_manager.memory:
        agent_runs = alert_manager.memory.runs
    ...
    # Get user input
    ####################################################################
    if prompt := st.chat_input("⚠️ How can I help with alert management?"):
    ...
    if last_message and last_message.get("role") == "user":
        ...
        with st.chat_message("assistant"):
            ...
            with st.spinner(":thinking_face: Processing alerts..."):
                response = ""
                try:
                    # Run the agent and stream the response
                    run_response = await alert_manager.arun(user_message, stream=True)
                    ...
                    if alert_manager.run_response is not None:
                        await add_message(agent_name, "assistant", response, alert_manager.run_response.tools)
                    else:
    ...
    # Session selector
    ####################################################################
    await session_selector(agent_name, alert_manager, get_alert_manager, user_id, model_id)

    ####################################################################
    # About section
    ####################################################################
    await utilities_widget(agent_name, alert_manager)
