...
async def header():
    st.markdown("<h1 class='heading'>🛡️ DD Guard</h1>", unsafe_allow_html=True)
    st.markdown(
        "<p class='subheading'>Real-time drawdown monitoring and risk protection system with kill-switch capabilities.</p>",
...
async def body() -> None:
    ...
    if (
        ...
        logger.info("---*--- Creating DD Guard Agent ---*---")
        dd_guard = get_dd_guard(user_id=user_id, model_id=model_id)
        st.session_state[agent_name]["agent"] = dd_guard
        st.session_state["selected_model"] = model_id
    else:
        dd_guard = st.session_state[agent_name]["agent"]

    ####################################################################
    # Load Agent Session from the database
    ####################################################################
    try:
        st.session_state[agent_name]["session_id"] = dd_guard.load_session()
    except Exception:
    ...
    ####################################################################
    if dd_guard.memory:
        agent_runs = dd_guard.memory.runs
    ...
    # Get user input
    ####################################################################
    if prompt := st.chat_input("🛡️ How can I help with drawdown protection?"):
    ...
    if last_message and last_message.get("role") == "user":
        ...
        with st.chat_message("assistant"):
            ...
            with st.spinner(":thinking_face: Analyzing drawdown risk..."):
                response = ""
                try:
                    # Run the agent and stream the response
                    run_response = await dd_guard.arun(user_message, stream=True)
                    ...
                    if dd_guard.run_response is not None:
                        await add_message(agent_name, "assistant", response, dd_guard.run_response.tools)
                    else:
    ...
    # Session selector
    ####################################################################
    await session_selector(agent_name, dd_guard, get_dd_guard, user_id, model_id)

    ####################################################################
    # About section
    ####################################################################
    await utilities_widget(agent_name, dd_guard)
