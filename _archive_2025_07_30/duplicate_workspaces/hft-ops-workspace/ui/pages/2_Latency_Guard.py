...
agent_name = "latency_guard"


async def header():
    st.markdown("<h1 class='heading'>⚡ Latency Guard</h1>", unsafe_allow_html=True)
    st.markdown(
        "<p class='subheading'>Ultra-low latency monitoring and protection system for execution performance.</p>",
...
async def body() -> None:
    ...
    if (
        ...
        logger.info("---*--- Creating Latency Guard Agent ---*---")
        latency_guard = get_latency_guard(user_id=user_id, model_id=model_id)
        st.session_state[agent_name]["agent"] = latency_guard
        st.session_state["selected_model"] = model_id
    else:
        latency_guard = st.session_state[agent_name]["agent"]

    ####################################################################
    # Load Agent Session from the database
    ####################################################################
    try:
        st.session_state[agent_name]["session_id"] = latency_guard.load_session()
    except Exception:
    ...
    ####################################################################
    if latency_guard.memory:
        agent_runs = latency_guard.memory.runs
    ...
    # Get user input
    ####################################################################
    if prompt := st.chat_input("⚡ How can I help with latency monitoring?"):
    ...
    if last_message and last_message.get("role") == "user":
        ...
        with st.chat_message("assistant"):
            ...
            with st.spinner(":thinking_face: Analyzing latency..."):
                response = ""
                try:
                    # Run the agent and stream the response
                    run_response = await latency_guard.arun(user_message, stream=True)
                    ...
                    if latency_guard.run_response is not None:
                        await add_message(agent_name, "assistant", response, latency_guard.run_response.tools)
                    else:
    ...
    # Session selector
    ####################################################################
    await session_selector(agent_name, latency_guard, get_latency_guard, user_id, model_id)

    ####################################################################
    # About section
    ####################################################################
    await utilities_widget(agent_name, latency_guard)
