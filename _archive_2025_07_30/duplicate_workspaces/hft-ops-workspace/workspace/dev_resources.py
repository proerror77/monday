...
# -*- Dev database running on port 5434:5432 (Ops workspace)
dev_db = PgVectorDb(
    name=f"{ws_settings.ws_name}-db",
    pg_user="ai",
    pg_password="ai",
    pg_database="ai",
    # Connect to this db on port 5434 (avoid conflict with other workspaces)
    host_port=5434,
)
...
# -*- Streamlit running on port 8503:8501 (Ops workspace)
...
# -*- FastApi running on port 8003:8000 (Ops workspace)
dev_fastapi = FastApi(
