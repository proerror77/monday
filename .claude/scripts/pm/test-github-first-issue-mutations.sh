...
case "$*" in
  'issue view 123 --json state --jq .state')
    if { [ "${GH_SCENARIO:-}" = close_state_read_fail ] && [ -f "${GH_STATE:?}" ]; } || [ "${GH_SCENARIO:-}" = reopen_state_read_fail ]; then exit 1; fi
    if [ -f "${GH_STATE:?}" ]; then cat "$GH_STATE"; else echo OPEN; fi ;;
  'issue view 123 --json blockedBy --jq'*) echo 0 ;;
  'issue view 123 --json labels --jq .labels[].name')
    case "${GH_SCENARIO:-}" in tracking*) echo tracking;; runtime*) echo runtime;; esac ;;
  'issue view 123 --json subIssues --jq'*) if [ "${GH_SCENARIO:-}" = tracking_child ]; then echo 1; else echo 0; fi ;;
  'issue view 123 --json body --jq .body') if [ "${GH_SCENARIO:-}" = tracking_box ]; then echo '  - [ ] pending'; else echo '- [x] done'; fi ;;
...
refuse tracking_box "$valid; Parent acceptance audit: passed"
...
for field in "Exact target" "Named controller" "Candidate identity" "Configuration identity" "Rollback identity" "Rollback procedure" "Stop rules" "Terminal result" "Cleanup evidence"; do
  ...
  refuse runtime "$valid; $missing"; refuse runtime "$valid; $placeholder"
done
allow normal "$valid"
