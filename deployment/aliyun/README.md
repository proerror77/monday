...
| Other governed services | `binance-usdm-reference-collector`, `bybit-options-archiver` active AND enabled AND `Result==success`, plus a restart-rate delta > 1 since the last poll |
| Production LOB restart delta | any positive `NRestarts` delta on `binance-lob-archiver-production@spot/usdm` since the last poll (`RuntimeMaxSec=infinity`; `Result` itself is hard gate 10) |
...
Production LOB `Result != success` is hard gate 10. The persistent-service
check still warns on any positive `NRestarts` delta for either Binance LOB
production instance between consecutive five-minute polls. Process health does
