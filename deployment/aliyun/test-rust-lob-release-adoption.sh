...
setup_fixture() {
  ...
  local usdm_symbols_config usdm_catalog
  ...
  install -m 0644 "$SCRIPT_DIR/binance-lob-archiver-production-usdm.env" \
  ...
  sed -e 's/^DATASET=.*/DATASET=usdm_perpetual_all/' \
    -e 's/^SYMBOLS=.*/SYMBOLS=ALL/' \
    "$SCRIPT_DIR/binance-lob-archiver-production-usdm.env" >"$USDM_ENV"
  chmod 0640 "$USDM_ENV"
