use ports::{Strategy, MarketEvent, OrderIntent, AccountView, VenueScope};
use hft_core::{Symbol, Side, OrderType, TimeInForce, Quantity};
use tracing::info;
...
impl Default for ImbalanceParams {
    fn default() -> Self {
        Self { obi_threshold: 0.2, lot: 0.01, top_levels: 5 }
    }
}
...
impl ImbalanceStrategy {
    ...
    pub fn with_name(symbol: Symbol, params: Option<ImbalanceParams>, strategy_name: String) -> Self {
        Self { 
            name: strategy_name,
            symbol, 
...
impl Strategy for ImbalanceStrategy {
    ...
    fn on_market_event(&mut self, event: &MarketEvent, _account: &AccountView) -> Vec<OrderIntent> {
        ...
        match event {
            MarketEvent::Snapshot(s) if &s.symbol == sym => {
                ...
                let bids: Vec<(f64, f64)> = s.bids.iter().map(|l| (l.price.to_f64().unwrap_or(0.0), l.quantity.to_f64().unwrap_or(0.0))).collect();
                let asks: Vec<(f64, f64)> = s.asks.iter().map(|l| (l.price.to_f64().unwrap_or(0.0), l.quantity.to_f64().unwrap_or(0.0))).collect();
                let obi = self.calc_obi(&bids, &asks);
                let qty = Quantity::from_f64(self.params.lot).unwrap_or_else(|_| Quantity::from_f64(0.01).unwrap());
                if obi > self.params.obi_threshold {
                    ...
                    info!("OBI {:.3} > {:.3}, BUY {}", obi, self.params.obi_threshold, sym.0);
                    ...
                    info!("OBI {:.3} < -{:.3}, SELL {}", obi, self.params.obi_threshold, sym.0);
                }
            }
            _ => {}
        }
    ...
    fn on_execution_event(&mut self, _event: &ports::ExecutionEvent, _account: &AccountView) -> Vec<OrderIntent> {
        Vec::new()
    }
...
pub fn create_imbalance_strategy(symbol: Symbol, params: Option<ImbalanceParams>) -> ImbalanceStrategy {
