...
impl VenueSnapshot {
    pub fn from_market_snapshot(venue: String, snapshot: &MarketSnapshot) -> Self {
        let best_bid = snapshot.bids.get(0).map(|level| (level.price, level.quantity));
        let best_ask = snapshot.asks.get(0).map(|level| (level.price, level.quantity));
...
impl ArbitrageStrategy {
    ...
    fn update_venue_snapshot(&mut self, venue: String, snapshot: &MarketSnapshot) {
        if snapshot.symbol == self.symbol {
            ...
            self.venue_snapshots.insert(venue_snapshot.venue.clone(), venue_snapshot);
        }
    }
    
    /// 清理陳舊資料
    fn cleanup_stale_data(&mut self, current_time: u64) {
        self.venue_snapshots.retain(|_, snapshot| {
            !snapshot.is_stale(current_time, self.config.max_stale_us)
        });
    }
    ...
    fn calculate_arbitrage(
        ...
        // 計算可交易數量 (取最小值)
        let max_quantity = buy_qty.0.min(sell_qty.0).min(
            rust_decimal::Decimal::from_f64(self.config.max_position_size)?
        );
    ...
    /// 根據套利信號生成訂單意圖
    fn signal_to_order_intents(&self, signal: ArbitrageSignal, account: &AccountView) -> Vec<OrderIntent> {
        ...
        if current_position.abs() + signal.quantity.0.to_f64().unwrap_or(0.0) > self.config.max_total_position {
...
impl Strategy for ArbitrageStrategy {
    ...
    fn initialize(&mut self) -> HftResult<()> {
        log::info!("套利策略初始化: 品種={}, 最小利差={}bps, 最大倉位={}", 
                   self.symbol.0, 
...
mod tests {
    ...
    fn test_parse_venue_symbol() {
        let (venue, symbol) = ArbitrageStrategy::parse_venue_symbol(&Symbol("BINANCE:BTCUSDT".to_string()));
