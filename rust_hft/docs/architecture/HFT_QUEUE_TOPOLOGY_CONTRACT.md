...
| --- | --- | --- | --- | --- | --- | --- |
| `raw_market_data_queue` | WS receiver | market-data engine | adapter/runner config | bounded; latest-wins or rebuild; never block engine | Bitget `latency_audit` supports std `sync-channel` and preallocated `spsc-spin`; generic Bitget `MarketStream`, Bitget zero-copy stream, and generic Binance stream are bounded; Binance still needs runner parity for Linux p99/p999 evidence | `cargo check -p hft-data-adapter-bitget --example latency_audit --locked` |
