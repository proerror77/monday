...
This guideline defines what Monday may call a latency improvement, what each timestamp means,
...
Never compare timestamps whose producer, clock, stream, endpoint, or capture boundary differs.
A field named `timestamp`, `received`, `submission`, or `ack` is not evidence of its boundary; the
code that records it defines the boundary.
...
| `exchange_trade_ts` (`T`) | Venue match/trade timestamp | Venue wall clock | Trade sequencing, features, and `T -> E` analysis |
| `userspace_frame_ts` | Complete WS message delivered by the WS library to Monday | Local monotonic | Start of userspace parse and processing spans |
...
| `write_return_ts` | Configured transport API returns success from that write call | Local monotonic | Userspace API write duration; not proof of kernel or NIC transmit |
...
packet-level proof. `intent -> write_return` may be reported as a userspace result but must not be
renamed as NIC transmit latency.
...
| P0 | `OrderIntentLifecycle::created_ts` is populated from the triggering market event's local receive time, not a strategy intent-emission timestamp. | Record `intent_ts` at intent emission and keep market receive time as separate causality provenance. | #399 |
