...
use crate::{
    Reconciler, RecoveryConfig, RecoveryError, RecoveryResult, RecoveryStats, StatePersistence,
    SystemSnapshot,
};
...
impl RecoveryManager {
    ...
    pub async fn perform_startup_recovery<T: ExecutionClient>(
        ...
        if let Some(snapshot) = self.persistence.load_latest_snapshot().await? {
            info!(
                "Found valid snapshot from {}, restoring state",
                snapshot.timestamp
            );
            ...
            // Restore Portfolio state
            {
                ...
                stats.positions_restored =
                    portfolio_guard.export_state().account_view.positions.len();
            }

            info!(
                "State restored: {} orders, {} positions",
        ...
        if self.config.enable_reconciliation && !execution_clients.is_empty() {
            ...
            for execution_client in execution_clients.iter_mut() {
                match self
                    .reconcile_with_client(oms.clone(), execution_client, &mut stats)
                    .await
                {
                    Ok(_) => {
        ...
        self.validate_recovered_state(oms.clone(), portfolio.clone())
            .await?;

        stats.recovery_duration_ms = start_time.elapsed().as_millis() as u64;

        info!(
            "Recovery completed in {}ms - {} orders, {} positions, {} reconciled",
    ...
    pub async fn reconcile_with_client<T: ExecutionClient>(
        ...
        info!(
            ...
            reconciliation_result.discrepancies.len()
        );
            ...
            match d {
                crate::reconciliation::Discrepancy::ExchangeOnly { exchange_order } => {
                    if self.config.auto_cancel_exchange_only {
                        info!(
                            "Auto-cancel exchange-only order: {}",
                ...
                crate::reconciliation::Discrepancy::LocalOnly { local_order } => {
                    if self.config.auto_mark_local_only_canceled {
                        info!(
                            "Mark local-only order as canceled: {}",
        ...
        for d in exch_state {
            match d {
                crate::reconciliation::Discrepancy::StatusMismatch {
                    local_order,
                    exchange_order,
                } => {
                    info!(
                    ...
                    match exchange_order.status {
                        ...
                        crate::reconciliation::ExchangeOrderStatus::Filled
                        | crate::reconciliation::ExchangeOrderStatus::PartiallyFilled => {
                            if exchange_order.filled_quantity.0 > local_order.cum_qty.0 {
                                let delta = hft_core::Quantity(
                                    exchange_order.filled_quantity.0 - local_order.cum_qty.0,
                                );
                                let fill_ev = ExecutionEvent::Fill {
                                    order_id: local_order.order_id.clone(),
                                    price: exchange_order.price.unwrap_or(
                                        local_order
                                            .avg_price
                                            .unwrap_or(hft_core::Price::from_f64(0.0).unwrap()),
                                    ),
                                    quantity: delta,
                                    timestamp: hft_core::now_micros(),
                                ...
                                // 向下修正：直接更新 OMS 累計數量
                                let _ = guard.update_filled_quantity(
                ...
                crate::reconciliation::Discrepancy::QuantityMismatch {
                    local_order,
                    exchange_order,
                } => {
                    info!(
                        "Recon-fix qty: {} local={} exch={}",
                        local_order.order_id.0,
                        local_order.cum_qty.0,
                        exchange_order.filled_quantity.0
                    );
                    let mut guard = oms.lock().await;
                    if exchange_order.filled_quantity.0 > local_order.cum_qty.0 {
                        let delta = hft_core::Quantity(
    ...
    async fn validate_recovered_state(
        ...
        // Basic validation checks
        {
            ...
            if account_view.cash_balance < 0.0 {
                warn!(
                    "Negative cash balance detected: {:.2}",
            ...
            for (symbol, position) in &account_view.positions {
                if position.quantity.0 != rust_decimal::Decimal::ZERO
            ...
            info!(
                "State validation completed: {} open orders, {} positions, cash: {:.2}",
                open_orders.len(),
        ...
        if !validation_errors.is_empty() {
            return Err(RecoveryError::StateInconsistency(format!(
                "Validation failed: {:?}",
                validation_errors
            )));
        }
...
mod tests {
    ...
    async fn test_save_and_recovery() {
        ...
        // Save snapshot
        manager
            .save_snapshot(oms.clone(), portfolio.clone(), "test-v1.0.0".to_string())
            .await
            .unwrap();
        ...
        let stats = manager
            .perform_startup_recovery(oms, portfolio, execution_clients.as_mut_slice())
            .await
            .unwrap();
