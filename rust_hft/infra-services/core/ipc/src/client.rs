...
impl IPCClient {
    ...
    pub async fn send_command(&self, command: Command) -> IPCResult<Response> {
        self.send_command_with_timeout(command, self.default_timeout)
            .await
    }
...
impl IPCClient {
    ...
    pub async fn cancel_orders_for_strategy(&self, strategy_id: String) -> IPCResult<Response> {
        self.send_command(Command::CancelOrdersForStrategy { strategy_id })
            .await
    }
}
