...
impl HttpClient {
    ...
    /// GET 請求
    pub async fn get(&self, path: &str, headers: Option<HashMap<String, String>>) -> Result<Response, reqwest::Error> {
    ...
    pub async fn post<T: Serialize>(
        ...
        let response = self.signed_request(Method::POST, path, headers, Some(body)).await?;
        Ok(response)
    }

    /// 解析 JSON 響應
    pub async fn parse_json<T: for<'de> Deserialize<'de>>(response: Response) -> Result<T, Box<dyn std::error::Error + Send + Sync>> {
