...
use risk::{DefaultRiskManager, EnhancedRiskManager, RiskConfig, EnhancedRiskConfig, TradingWindow};
use chrono::{DateTime, Utc, Weekday};

use crate::{RiskConfig as SystemRiskConfig, StrategyRiskOverride, EnhancedRiskSettings, TradingWindowConfig};
...
impl RiskManagerFactory {
    ...
    pub fn create_risk_manager(
        system_risk_config: &SystemRiskConfig,
    ) -> Box<dyn RiskManager> {
        match system_risk_config.risk_type.as_str() {
            "Enhanced" => {
                let enhanced_config = system_risk_config.enhanced.as_ref()
                    .cloned()
                    .unwrap_or_default();
    ...
    fn convert_trading_window(config: &Option<TradingWindowConfig>) -> Option<TradingWindow> {
        config.as_ref().map(|tw| {
            TradingWindow {
                start_hour_utc: tw.start_hour_utc,
                end_hour_utc: tw.end_hour_utc,
                allowed_weekdays: tw.allowed_weekdays.iter()
                ...
                market_holidays: tw.market_holidays.iter()
                    .filter_map(|date_str| {
...
impl StrategyAwareRiskManager {
    pub fn new(
        ...
        info!("创建策略感知风控管理器，包含 {} 个策略覆盖配置", strategy_overrides.len());
        Self {
            base_risk_manager,
            strategy_overrides,
        }
    ...
    fn apply_strategy_overrides(&self, strategy_name: &str, mut intents: Vec<ports::OrderIntent>) -> Vec<ports::OrderIntent> {
...
impl RiskManager for StrategyAwareRiskManager {
    fn review(
        ...
        for intent in intents {
            strategy_groups.entry(intent.strategy_id.clone()).or_default().push(intent);
        }
        ...
        for (strategy_id, mut strategy_intents) in strategy_groups {
            ...
            #[cfg(feature = "infra-metrics")]
            {
                ...
                if override_applied {
                    debug!("策略 {} 應用了 {} 個訂單意圖的風控覆蓋", strategy_id, strategy_intents.len());
                }
            }
        ...
        // 最後通過基礎風控管理器進行統一檢查
        self.base_risk_manager.review(processed_intents, account, venue)
    }
    ...
    fn review_orders(
        ...
        // 最後通過基礎風控管理器進行統一檢查
        self.base_risk_manager.review_orders(processed_intents, account, venue_specs)
    }
...
mod tests {
    ...
    fn test_risk_manager_factory_default() {
        let mut risk_config = SystemRiskConfig {
        ...
        // The risk manager should be created successfully
        // We can't inspect the internal type easily, but creation success indicates it works
        assert!(!std::ptr::eq(&*risk_manager as *const dyn RiskManager, std::ptr::null()));
    }
    ...
    fn test_risk_manager_factory_enhanced() {
        ...
        // The enhanced risk manager should be created successfully
        assert!(!std::ptr::eq(&*risk_manager as *const dyn RiskManager, std::ptr::null()));
    }
    ...
    fn test_strategy_aware_risk_manager() {
        ...
        overrides.insert("strategy1".to_string(), StrategyRiskOverride {
        ...
        let strategy_aware_manager = RiskManagerFactory::create_strategy_aware_risk_manager(&config_with_overrides);
        
        // Should create the wrapper successfully
        assert!(!std::ptr::eq(&*strategy_aware_manager as *const dyn RiskManager, std::ptr::null()));
    }
}