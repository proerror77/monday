...
impl BinanceRestClient {
    ...
    pub async fn get_depth(&self, symbol: &Symbol, limit: Option<u16>) -> HftResult<DepthSnapshot> {
        ...
        if !response.status().is_success() {
            ...
            if let Ok(error) = serde_json::from_str::<BinanceError>(&body) {
                return Err(HftError::Exchange(format!("Binance API 錯誤 {}: {} ({})", 
                    status, error.msg, error.code)));
            }
            
            return Err(HftError::Network(format!("API 請求失敗 {}: {}", status, body)));
        }
        ...
        info!("獲取 {} 深度快照成功，序號: {}, 買檔: {}, 賣檔: {}", 
            symbol, snapshot.last_update_id, snapshot.bids.len(), snapshot.asks.len());
    ...
    pub async fn ping(&self) -> HftResult<()> {
        ...
        if response.status().is_success() {
            ...
            Err(HftError::Network(format!("ping 失敗: {}", response.status())))
        }
    }
    ...
    pub async fn get_server_time(&self) -> HftResult<u64> {
        ...
        if !response.status().is_success() {
            return Err(HftError::Network(format!("獲取時間失敗: {}", response.status())));
        }
    ...
    pub async fn get_exchange_info(&self) -> HftResult<serde_json::Value> {
        ...
        if !response.status().is_success() {
            return Err(HftError::Network(format!("獲取交易所信息失敗: {}", response.status())));
        }
