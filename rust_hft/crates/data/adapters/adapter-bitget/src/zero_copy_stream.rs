...
use simd_json::prelude::{ValueObjectAccess, ValueAsContainer, ValueAsScalar};
...
impl ZeroCopyBitgetStream {
    ...
    /// 零拷貝解析訂單簿數據
    fn parse_orderbook_zero_copy(&mut self, json_bytes: &[u8], symbol: &Symbol) -> Result<MarketSnapshot, HftError> {
        // 使用 simd-json 借用模式解析
        ...
        let data = value.get("data")
            ...
            .ok_or_else(|| HftError::Generic { message: "無效的訂單簿數據".to_string() })?;

        // 統一時間戳處理 - 區分交易所和本地時間戳
        let exchange_ts = data.get("ts")
    ...
    fn parse_trade_zero_copy(&mut self, json_bytes: &[u8]) -> Result<Trade, HftError> {
        ...
        let data = value.get("data")
            .and_then(|d| d.as_array()?.get(0))
            .ok_or_else(|| HftError::Generic { message: "無效的成交數據".to_string() })?;

        let symbol_str = data.get("instId")
            .and_then(|s| s.as_str())
            .unwrap_or("");
...
impl MessageHandler for ZeroCopyMessageHandler {
    fn handle_connected(&mut self, _client: &mut integration::ws::WsClient) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        ...
        if let Some(arg) = value.get("arg") {
            ...
            match channel {
                "books15" => {
                    ...
                    // 提取交易所時間戳並創建統一時間戳
                    let exchange_ts = value.get("data")
