...
use crate::{Command, Response, ResponseData, SystemStatus, AccountInfo, Position, Order, TradingMode};
    ...
    async fn handle_command(&self, command: Command) -> Response {
        match command {
            ...
            Command::LoadModel { model_path, model_version, sha256_hash } => {
                ...
                Response::Error {
                    message: format!("Model loading not implemented: {} v{}", model_path, model_version),
                    code: Some(501),
                }
            }
            Command::UpdateRisk { .. } => {
            ...
            Command::CancelOrder { order_id: _, symbol: _ } => Response::Ok,
            ...
            Command::SetStrategyEnabled { strategy_id, enabled } => {
                ...
                Response::Error {
                    message: format!("Set strategy {} enabled={} not implemented yet", strategy_id, enabled),
                    code: Some(501),
                }
            }
...
impl CommandHandler for MockCommandHandler {
    async fn handle_command(&self, command: Command) -> Response {
        match command {
            ...
            Command::LoadModel { model_version, .. } => {
                Response::Ok
            }
