...
use crate::{IPCMessage, IPCPayload, Command, Response, IPCError, IPCResult, MAX_MESSAGE_SIZE};
...
impl<H: CommandHandler + Send + Sync + 'static> IPCServer<H> {
    ...
    pub fn new_with_auth<P: AsRef<Path>>(socket_path: P, handler: H, auth_token: Option<String>) -> Self {
    ...
    pub fn shutdown(&self) -> IPCResult<()> {
        self.shutdown_tx.send(()).map_err(|_| {
            IPCError::Handler("Failed to send shutdown signal".to_string())
        })?;
        Ok(())
    }
    ...
    pub fn broadcast_status(&self, message: IPCMessage) -> IPCResult<()> {
        self.status_tx.send(message).map_err(|_| {
            IPCError::Handler("No active clients to broadcast to".to_string())
        })?;
        Ok(())
    }
    ...
    async fn write_message(stream: &mut UnixStream, message: &IPCMessage) -> IPCResult<()> {
        ...
        if message_data.len() > MAX_MESSAGE_SIZE {
            return Err(IPCError::Handler(
                format!("Response too large: {} bytes", message_data.len())
            ));
        }
