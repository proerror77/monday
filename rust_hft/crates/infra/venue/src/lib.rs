...
use hft_core::{HftResult, HftError, Price, Quantity, Symbol};
use ports::VenueSpec;
...
impl VenueSpecManager {
    ...
    pub fn validate_order(
        ...
        if quantity < spec.min_qty {
            return Err(HftError::InvalidOrder(format!(
                "數量過小: {} < 最小數量 {}", quantity.0, spec.min_qty.0
            )));
        }
        ...
        if let Some(max_qty) = spec.max_quantity {
            if quantity > max_qty {
                return Err(HftError::InvalidOrder(format!(
                    "數量過大: {} > 最大數量 {}", quantity.0, max_qty.0
                )));
            }
        }
        ...
        if let Some(price) = price {
            ...
            if (price_decimal % tick_decimal) != rust_decimal::Decimal::ZERO {
                return Err(HftError::InvalidOrder(format!(
                    "價格不符合步進: {} 不是 {} 的倍數", price_decimal, tick_decimal
                )));
            }
            ...
            if notional < spec.min_notional {
                return Err(HftError::InvalidOrder(format!(
                    "名義值過小: {} < 最小名義值 {}", notional, spec.min_notional
                )));
            }
        }
    ...
    /// 標準化數量到合法步進
    pub fn normalize_quantity(&self, venue: &str, symbol: &Symbol, quantity: Quantity) -> Option<Quantity> {
        let spec = self.get_spec(venue, symbol)?;
