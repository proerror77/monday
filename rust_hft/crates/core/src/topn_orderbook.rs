...
impl<const N: usize> TopNOrderBook<N> {
    ...
    pub fn apply_snapshot(&mut self, 
                         bids: &[(f64, f64)], 
                         asks: &[(f64, f64)], 
                         timestamp: Timestamp) -> Result<(), HftError> {
        self.clear();
...
impl<const N: usize> fmt::Display for TopNOrderBook<N> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        writeln!(f, "TopN{} OrderBook [{}] (v{})", N, self.symbol, self.version)?;
        writeln!(f, "Timestamp: {}", self.last_update_ts)?;
        
        // 顯示 asks（從高到低，方便視覺對齊）
        for i in (0..self.ask_count).rev() {
            writeln!(f, "  Ask[{}]: {:>12.4} @ {:>10.4}", 
...
mod tests {
    ...
    fn test_topn_orderbook_updates() {
        ...
        assert!((spread - 0.3).abs() < 1e-10, "Spread should be approximately 0.3, got {}", spread);
    }
    ...
    fn test_dual_joiner_arbitrage() {
        ...
        joiner.venue_a.apply_snapshot(&bids_a, &asks_a, 1000).unwrap();
        
        // Bitget: bid=67203, ask=67204 （比 Binance 高）
        let bids_b = vec![(67203.0, 1.0)];
        let asks_b = vec![(67204.0, 1.0)];
        joiner.venue_b.apply_snapshot(&bids_b, &asks_b, 1000).unwrap();
