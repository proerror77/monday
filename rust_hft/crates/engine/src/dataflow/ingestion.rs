...
pub struct IngestionMetrics {
    ...
    pub flip_count: u64,
    /// 攝取階段延遲統計（微秒）
    pub ingestion_latency_micros: Vec<u64>,
    /// 最近的延遲測量（用於實時監控）
...
impl EventIngester {
    ...
    pub fn ingest(&mut self, event: MarketEvent) -> Result<(), HftError> {
        ...
        if let Some(event_ts) = self.extract_timestamp(&event) {
            // 如果事件有時間戳，使用事件時間戳作為起始時間
            tracker = LatencyTracker::from_time(event_ts);
        }
        ...
        if let Some(event_ts) = self.extract_timestamp(&event) {
            ...
            if delay > self.config.stale_threshold_us {
                ...
                warn!("丟棄陳舊事件: {}μs 延遲 > {}μs 閾值", delay, self.config.stale_threshold_us);
                return Ok(()); // 靜默丟棄陳舊事件
            }
        }
...
impl EventConsumer {
    ...
    fn update_symbol_state(&mut self, event: &MarketEvent) {
        let (symbol, sequence) = match event {
            ...
            MarketEvent::Arbitrage(a) => (a.leg1.clone(), 0), // Arbitrage 使用 leg1 作為 symbol
            MarketEvent::Disconnect { .. } => return,
        };
        
        let state = self.symbol_state.entry(symbol).or_insert_with(|| SymbolState {
...
mod tests {
    use super::*;
    use ports::{MarketSnapshot, BookLevel};
    use hft_core::{Symbol, Price, Quantity};
    ...
    fn test_ingestion_basic() {
        ...
        assert!(matches!(received_events[0], MarketEvent::Snapshot(_)));
    }
    ...
    fn test_stale_event_rejection() {
        ...
        // 應該被拒絕但不返回錯誤
        assert!(ingester.ingest(MarketEvent::Snapshot(stale_snapshot)).is_ok());
