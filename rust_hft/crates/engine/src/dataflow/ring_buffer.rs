...
impl<T> SpscRingBuffer<T> {
    pub fn new(capacity: usize) -> Self {
        ...
        for _ in 0..actual_capacity { buffer.push(None); }
        Self {
    ...
    pub fn try_push(&self, item: T) -> Result<(), T> {
        ...
        if next_head == tail { return Err(item); }
        unsafe { let slot = &mut *self.buffer.as_ptr().add(head).cast_mut(); *slot = Some(item); }
    ...
    pub fn try_pop(&self) -> Option<T> {
        ...
        let item = unsafe { let slot = &mut *self.buffer.as_ptr().add(tail).cast_mut(); slot.take() };
        let next_tail = (tail + 1) & (self.capacity - 1);
        self.tail.store(next_tail, Ordering::Release);
        item
    }
    pub fn utilization(&self) -> f64 {
        let head = self.head.load(Ordering::Relaxed);
        let tail = self.tail.load(Ordering::Relaxed);
        let used = if head >= tail { head - tail } else { self.capacity - (tail - head) };
        let max_usable = self.capacity - 1; // Ring buffer reserves one slot to distinguish full vs empty
        used as f64 / max_usable as f64
    }
    pub fn is_empty(&self) -> bool { self.head.load(Ordering::Acquire) == self.tail.load(Ordering::Acquire) }
...
pub struct SpscProducer<T> { ring_buffer: Arc<SpscRingBuffer<T>> }
impl<T> SpscProducer<T> {
    pub fn send(&self, item: T) -> Result<(), T> { self.ring_buffer.try_push(item) }
    pub fn force_send(&self, item: T) -> bool { self.ring_buffer.force_push(item) }
    pub fn is_full(&self) -> bool { self.ring_buffer.is_full() }
    pub fn utilization(&self) -> f64 { self.ring_buffer.utilization() }
}
impl<T> Clone for SpscProducer<T> { fn clone(&self) -> Self { Self { ring_buffer: self.ring_buffer.clone() } } }

pub struct SpscConsumer<T> { ring_buffer: Arc<SpscRingBuffer<T>> }
impl<T> SpscConsumer<T> {
    pub fn recv(&self) -> Option<T> { self.ring_buffer.try_pop() }
    pub fn is_empty(&self) -> bool { self.ring_buffer.is_empty() }
    pub fn utilization(&self) -> f64 { self.ring_buffer.utilization() }
}
...
pub fn spsc_ring_buffer<T>(capacity: usize) -> (SpscProducer<T>, SpscConsumer<T>) {
    ...
    (SpscProducer { ring_buffer: ring_buffer.clone() }, SpscConsumer { ring_buffer })
}
