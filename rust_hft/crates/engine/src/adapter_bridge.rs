...
use crate::dataflow::{EventIngester, EventConsumer, IngestionConfig};
...
impl AdapterBridge {
        ...
        S: MarketStream + Send + 'static,
    {
        ...
        // 訂閱數據流
        let event_stream = stream.subscribe(symbols).await
            .map_err(|e| HftError::Generic { message: format!("訂閱失敗: {}", e) })?;
                    ...
                    event_result = event_stream.next() => {
                        match event_result {
                            Some(Ok(event)) => {
                                ...
                                // 每個事件都記錄，以便調試
                                tracing::info!("AdapterBridge 收到事件 #{}: {:?}", 
                                              events_processed,
                                              match &event {
                                                  ports::MarketEvent::Bar(bar) => format!("Bar({}, close={:.2})", bar.symbol.0, bar.close.0.to_f64().unwrap_or(0.0)),
