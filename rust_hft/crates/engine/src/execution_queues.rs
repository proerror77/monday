...
use crate::dataflow::ring_buffer::{spsc_ring_buffer, SpscProducer, SpscConsumer};
use ports::{OrderIntent, ExecutionEvent};
...
impl EngineQueues {
    ...
    /// 批量发送订单意图
    pub fn send_intents_batch(&mut self, intents: Vec<OrderIntent>) -> Vec<OrderIntent> {
        let mut failed = Vec::new();
        
        for intent in intents {
            if let Err(failed_intent) = self.send_intent(intent) {
                failed.push(failed_intent);
            }
        }
    ...
    /// 接收执行回报 (非阻塞批量)
    pub fn receive_events(&mut self) -> Vec<ExecutionEvent> {
