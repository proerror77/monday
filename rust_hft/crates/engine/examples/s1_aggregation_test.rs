...
use engine::aggregation::{AggregationEngine, TopNSnapshot, BarBuilder, MarketView};
use hft_core::*;
use ports::*;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("✅ TopN 快照更新成功:");
    println!("   - 買方 5 檔: {:?}", topn.bid_prices.iter().map(|p| p.0.to_f64().unwrap_or(0.0)).collect::<Vec<_>>());
    println!("   - 賣方 5 檔: {:?}", topn.ask_prices.iter().map(|p| p.0.to_f64().unwrap_or(0.0)).collect::<Vec<_>>());
    println!("   - 中間價: {:?}", topn.get_mid_price().map(|p| p.0.to_f64().unwrap_or(0.0)));
    println!("   - 價差: {:?} bps", topn.get_spread_bps().map(|b| b.0.to_f64().unwrap_or(0.0)));

    // 2. 測試 BarBuilder K線聚合
    println!("\n📈 測試 BarBuilder K線聚合...");
    let mut bar_builder = BarBuilder::new(Symbol("BTCUSDT".to_string()), 60000, current_timestamp_us());
    ...
    let should_flip = agg_engine.handle_event(snapshot_event)?;
    println!("   處理快照事件，觸發快照發佈: {}", should_flip);
    
    // 處理成交事件
    let trade_event = MarketEvent::Trade(generator.generate_trade(Side::Buy));
    let should_flip = agg_engine.handle_event(trade_event)?;
    println!("   處理成交事件，觸發快照發佈: {}", should_flip);
    ...
    println!("   - 套利機會數量: {}", market_view.arbitrage_opportunities.len());
    println!("   - 時間戳: {}", market_view.timestamp);
    
    // 測試 MarketView 查詢方法
    let symbol = Symbol("BTCUSDT".to_string());
    if let Some((bid_price, bid_qty)) = market_view.get_best_bid(&symbol) {
        println!("   - 最佳買價: {:.2} @ {:.3}", 
                bid_price.0.to_f64().unwrap_or(0.0),
                bid_qty.0.to_f64().unwrap_or(0.0));
    }
    
    if let Some((ask_price, ask_qty)) = market_view.get_best_ask(&symbol) {
        println!("   - 最佳賣價: {:.2} @ {:.3}", 
    ...
    if let Some(spread_bps) = market_view.get_spread_bps(&symbol) {
    ...
    // 橋接並運行幾個 tick
    let consumer = adapter_bridge.bridge_stream(mock_adapter, vec![Symbol("BTCUSDT".to_string())]).await?;
    ...
    for i in 0..5 {
        ...
        if result.snapshot_published {
            ...
            println!("   Tick #{}: {} 事件, {} 訂單簿", 
                    i + 1, result.events_total, view.orderbooks.len());
        }
