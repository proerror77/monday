                    ...
                    Ok(result) => {
                        ...
                        if result.snapshot_published {
                            ...
                            if orderbooks_count > 0 {
                                info!(
                                    "📸 快照 #{}: {} 新事件, {} 訂單簿, 時間戳: {}",
                                    total_snapshots, result.events_total, orderbooks_count, market_view.timestamp
                                );
                                
                                // 顯示訂單簿摘要
                                for (symbol, _ob) in &market_view.orderbooks {
                                    if let (Some(best_bid), Some(best_ask)) = (
                                        market_view.get_best_bid(symbol),
                                        market_view.get_best_ask(symbol)
                                    ) {
                                ...
                                debug!("📸 快照 #{}: {} 事件但無訂單簿數據", total_snapshots, result.events_total);
                            }
                        }
                        ...
                        if now.duration_since(last_report_time) >= Duration::from_secs(5) {
                            ...
                            if events_delta > 0 {
                                ...
                                info!(
                                    ...
                                    events_per_sec, total_events, total_snapshots, stats.consumers_count
                                );
                            } else {
                                info!(
