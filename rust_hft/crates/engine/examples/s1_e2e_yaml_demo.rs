...
impl EngineConfigYaml {
    fn to_engine_config(&self) -> EngineConfig {
        let symbols: Vec<Symbol> = self.aggregation.symbols
            .iter()
...
impl StatisticsCollector {
    ...
    fn report(&mut self) -> (f64, f64) {
        ...
        println!("   事件吞吐: {:.1} 事件/秒 (總計 {} 事件)", events_per_sec, self.total_events);
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let config: S1Config = serde_yaml::from_str(&config_content)
        .map_err(|e| format!("YAML 解析錯誤: {}", e))?;
    ...
    if config.strategies.trend.enabled {
        ...
        let mut trend_strategy = TrendStrategy::new(
            Symbol(config.strategies.trend.symbol.clone()), 
            trend_config
        );
        trend_strategy.initialize()?;
        strategies.push(("趨勢策略".to_string(), Box::new(trend_strategy) as Box<dyn Strategy>));
        println!("   ✅ 趨勢策略已啟用");
    }
    ...
    if config.strategies.arbitrage.enabled {
        ...
        strategies.push(("套利策略".to_string(), Box::new(arbitrage_strategy) as Box<dyn Strategy>));
        println!("   ✅ 套利策略已啟用");
    }
    ...
    // 7. 運行主循環
    println!("\n🔄 開始 E2E 主循環 ({}秒)...", config.runtime.duration_sec);
