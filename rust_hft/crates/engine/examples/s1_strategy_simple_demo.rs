...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut arbitrage_strategy = ArbitrageStrategy::new(Symbol("BTCUSDT".to_string()), arbitrage_config);
    ...
    // 4. 模擬 K 線數據 - 創建一個上升趨勢
    let base_prices = vec![50000.0, 50100.0, 50050.0, 50200.0, 50180.0, 50300.0, 50280.0, 50400.0, 50350.0, 50500.0];
    ...
    for (i, &price) in base_prices.iter().enumerate() {
        ...
        if !orders.is_empty() {
            ...
            println!("  K線 #{}: 收盤價 {:.0}, 生成 {} 筆訂單", 
                     i + 1, price, orders.len());
    ...
    for (i, &(binance_price, bitget_price)) in price_scenarios.iter().enumerate() {
        ...
        if !orders.is_empty() {
            ...
            println!("  場景 #{}: Binance {:.0}, Bitget {:.0}, 價差 {:.0} ({:.1}bps), 生成 {} 筆訂單", 
                     i + 1, binance_price, bitget_price, spread, spread_bps, orders.len());
            ...
            println!("  場景 #{}: Binance {:.0}, Bitget {:.0}, 價差 {:.0} ({:.1}bps), 無套利機會", 
                     i + 1, binance_price, bitget_price, spread, spread_bps);
        }
    }
