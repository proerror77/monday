...
use ports::{MarketEvent, MarketSnapshot, BookLevel, Strategy, AccountView, OrderIntent, ExecutionEvent};
use hft_core::{OrderType, TimeInForce, Side};
...
fn bench_hotpath_latency() {
    ...
    let n = 50_000u32;
    {
        ...
        for i in 0..n {
            let venue = if i % 2 == 0 { VenueId::BINANCE } else { VenueId::BITGET };
            let sym = if venue == VenueId::BINANCE { Symbol("BINANCE:BTCUSDT".to_string()) } else { Symbol("BITGET:BTCUSDT".to_string()) };
    ...
    while idle_spins < 2000 {
        ...
        if let Ok(res) = &tick {
            if res.events_total == 0 && res.execution_events_processed == 0 && res.orders_generated == 0 {
                idle_spins += 1;
            } else {
                idle_spins = 0;
            }
    ...
    if let Some(e2e) = stats.get(&hft_core::LatencyStage::EndToEnd) {
        println!("Hotpath EndToEnd: count={} p50={}us p95={}us p99={}us", e2e.count, e2e.p50_micros, e2e.p95_micros, e2e.p99_micros);
    }
    if let Some(strategy) = stats.get(&hft_core::LatencyStage::Strategy) {
        println!("Strategy: count={} p50={}us p95={}us p99={}us", strategy.count, strategy.p50_micros, strategy.p95_micros, strategy.p99_micros);
    }
