...
use hft_core::{HftResult, HftError, Price, Quantity, Timestamp};
...
impl FeatureExtractor {
    ...
    fn convert_to_topn(&self, snapshot: &ports::MarketSnapshot) -> HftResult<TopNSnapshot> {
        let n = self.config.top_n.min(snapshot.bids.len()).min(snapshot.asks.len());
        
        if n == 0 {
            return Err(HftError::Parse("訂單簿為空".to_string()));
        }
        
        let mut bids: Vec<(f64, f64)> = snapshot.bids.iter()
            .take(n)
            .map(|level| (
        ...
        let mut asks: Vec<(f64, f64)> = snapshot.asks.iter()
            .take(n)
            .map(|level| (
                level.price.0.to_string().parse::<f64>().unwrap_or(0.0),
                level.quantity.0.to_string().parse::<f64>().unwrap_or(0.0)
    ...
    fn process_topn_features(&mut self, topn: &TopNSnapshot) -> HftResult<Option<Array1<f32>>> {
        ...
        debug!("提取特徵完成: {} 維，總計 {} 次", normalized_features.len(), self.feature_count);
    ...
    fn calculate_imbalance(&self, topn: &TopNSnapshot) -> f64 {
        ...
        (topn.total_bid_volume - topn.total_ask_volume) / (topn.total_bid_volume + topn.total_ask_volume)
    }
    ...
    fn update_windows(&mut self, topn: &TopNSnapshot, imbalance: f64) {
        ...
        self.volume_window.push_back(topn.total_bid_volume + topn.total_ask_volume);
        ...
        self.volume_stats.update(topn.total_bid_volume + topn.total_ask_volume);
    ...
    fn extract_price_features(&self, topn: &TopNSnapshot) -> HftResult<Vec<f32>> {
        ...
        // 價格變化率 (如果有歷史數據)
        if let Some(&last_price) = self.price_window.get(self.price_window.len().saturating_sub(2)) {
    ...
    fn extract_volume_features(&self, topn: &TopNSnapshot) -> HftResult<Vec<f32>> {
        ...
        // 量變化率
        if let Some(&last_volume) = self.volume_window.get(self.volume_window.len().saturating_sub(2)) {
    ...
    fn normalize_features(&self, features: Vec<f32>) -> HftResult<Array1<f32>> {
        ...
        match self.config.normalization.as_str() {
            ...
            "minmax" => {
                ...
                if let (Some(&min_val), Some(&max_val)) = (normalized.min(), normalized.max()) {
            ...
            _ => {
                warn!("未知的歸一化方法: {}，跳過歸一化", self.config.normalization);
            }
        }
...
impl FeaturePipeline {
    ...
    /// 處理市場事件並提取特徵
    pub fn process_event(&mut self, symbol: &str, event: &MarketEvent) -> HftResult<Option<Array1<f32>>> {
        if let Some(extractor) = self.extractors.get_mut(symbol) {
            extractor.extract_features(event)
        } else {
            Err(HftError::Config(format!("未找到品種的特徵提取器: {}", symbol)))
        }
    }
