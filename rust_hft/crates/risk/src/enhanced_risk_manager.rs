...
use hft_core::{Symbol, HftError, Quantity, Timestamp};
...
impl EnhancedRiskManager {
    ...
    fn check_circuit_breaker(&mut self, account: &AccountView) -> Result<(), HftError> {
        ...
        // 檢查觸發條件
        let current_balance = Decimal::try_from(account.cash_balance + account.unrealized_pnl)
            .unwrap_or_default();
        
        let daily_loss = self.stats.starting_balance - current_balance;
        let drawdown_pct = if self.stats.peak_balance > Decimal::ZERO {
            ((self.stats.peak_balance - current_balance) / self.stats.peak_balance * Decimal::from(100))
    ...
    fn trigger_circuit_breaker(&mut self, reason: String) {
        ...
        self.circuit_breaker.recovery_time = Some(
            now + self.config.cb_recovery_time_minutes * 60 * 1_000_000
        );
        ...
        // 記錄到拒絕原因統計
        *self.stats.rejection_reasons.entry("circuit_breaker".to_string()).or_insert(0) += 1;
    }
    ...
    fn check_cooldown(&mut self, symbol: &Symbol) -> Result<(), HftError> {
        ...
        if now - self.stats.last_global_order_time < self.config.global_order_cooldown_ms * 1000 {
            let remaining_ms = (self.config.global_order_cooldown_ms * 1000 - 
                               (now - self.stats.last_global_order_time)) / 1000;
        ...
        if let Some(&last_time) = self.stats.last_symbol_order_times.get(symbol) {
            if now - last_time < self.config.symbol_order_cooldown_ms * 1000 {
                let remaining_ms = (self.config.symbol_order_cooldown_ms * 1000 - 
                                   (now - last_time)) / 1000;
    ...
    fn update_stats_after_order(&mut self, symbol: &Symbol) {
        ...
        self.stats.last_symbol_order_times.insert(symbol.clone(), now);
    }

    /// 記錄拒絕原因
    fn record_rejection(&mut self, reason: &str) {
        self.stats.total_rejected_today += 1;
        *self.stats.rejection_reasons.entry(reason.to_string()).or_insert(0) += 1;
        warn!("訂單被風控拒絕: {}", reason);
    }
    ...
    pub fn get_risk_report(&self) -> RiskReport {
        RiskReport {
            ...
            rejection_rate: if self.stats.total_orders_today > 0 {
                self.stats.total_rejected_today as f64 / self.stats.total_orders_today as f64 * 100.0
            } else {
                0.0
            },
...
impl RiskManager for EnhancedRiskManager {
    ...
    fn get_risk_metrics(&self) -> HashMap<String, f64> {
        ...
        m.insert("orders_this_second".into(), self.stats.orders_this_second as f64);
        m.insert("orders_this_minute".into(), self.stats.orders_this_minute as f64);
        m.insert("orders_this_hour".into(), self.stats.orders_this_hour as f64);
        m
    }
