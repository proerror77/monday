...
use hft_core::{Symbol, HftError, Timestamp, now_micros, Side, OrderType, TimeInForce, Price, Quantity, VenueId};
...
impl SimplifiedProfessionalRiskManager {
    ...
    /// 執行專業風控檢查
    pub fn execute_risk_check(&mut self, 
                             intent: &OrderIntent, 
                             account: &AccountView) -> SimplifiedRiskDecision {
    ...
    fn check_basic_limits(&self, intent: &OrderIntent, account: &AccountView) -> Option<String> {
        ...
        if order_notional > self.config.max_order_notional {
            return Some(format!("單筆訂單名義價值超限: {:.2} > {:.2}", 
        ...
        if total_notional > self.config.max_global_notional {
            return Some(format!("全局名義價值超限: {:.2} > {:.2}", 
    ...
    fn check_rate_limits(&mut self, intent: &OrderIntent, current_time: Timestamp) -> Option<String> {
        // 清理過期記錄
    ...
    fn trigger_circuit_breaker(&mut self, reason: &str) {
        ...
        self.circuit_breaker_recovery_time = current_time + (self.config.recovery_time_minutes * 60 * 1_000_000);
        
        error!("觸發熔斷器: {} (恢復時間: {} 分鐘)", reason, self.config.recovery_time_minutes);
    }
    ...
    fn increment_reject_reason(&mut self, reason: &str) {
        *self.stats.reject_reasons.entry(reason.to_string()).or_insert(0) += 1;
    }
...
impl RiskManager for SimplifiedProfessionalRiskManager {
    fn review_orders(&mut self, 
                    ...
                    account: &AccountView,
                    _venue_specs: &HashMap<String, VenueSpec>) -> Vec<OrderIntent> {
    ...
    fn review(&mut self, 
             intents: Vec<OrderIntent>, 
             account: &AccountView,
             _venue: &VenueSpec) -> Vec<OrderIntent> {
        let mut approved_orders = Vec::new();
    ...
    fn on_execution_event(&mut self, event: &ExecutionEvent) {
        // 更新 PNL 跟踪（簡化實現）
        match event {
            ExecutionEvent::Fill { price, quantity, .. } => {
    ...
    fn get_risk_metrics(&self) -> HashMap<String, f64> {
        ...
        if total > 0.0 {
            metrics.insert("approval_rate".to_string(), self.stats.orders_allowed as f64 / total);
            metrics.insert("rejection_rate".to_string(), self.stats.orders_rejected as f64 / total);
        }
        
        metrics.insert("circuit_breaker_active".to_string(), if self.circuit_breaker_active { 1.0 } else { 0.0 });
