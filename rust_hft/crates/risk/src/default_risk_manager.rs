...
use hft_core::{Symbol, HftError, Price, Quantity};
...
impl DefaultRiskManager {
    ...
    fn check_position_limits(
        ...
        // 获取当前持仓
        let current_position = account.positions
            .get(&intent.symbol)
            .map(|p| p.quantity)
        ...
        if let Some(price) = &intent.price {
            ...
            let total_notional = account.positions.values()
                .map(|p| p.avg_price.0 * p.quantity.0.abs())
                .sum::<rust_decimal::Decimal>() + notional;
...
impl RiskManager for DefaultRiskManager {
    fn review(
        ...
        for mut intent in intents {
            ...
            if self.config.aggressive_mode {
                ...
                intent.quantity = PrecisionNormalizer::normalize_quantity(intent.quantity, venue.lot_size);
            ...
            // 第二步：交易所规则验证
            let venue_check = PrecisionNormalizer::validate_order_minimums(&intent, venue.min_qty, venue.min_notional);
        ...
        if self.total_orders_today % 100 == 0 && self.total_orders_today > 0 {
            info!(
                ...
                (self.total_orders_today - self.total_rejected_today) as f64 
                    / self.total_orders_today as f64 * 100.0
            );
        }
    ...
    fn on_execution_event(&mut self, event: &ExecutionEvent) {
        ...
        if let ExecutionEvent::Fill { price, quantity, .. } = event {
    ...
    fn get_risk_metrics(&self) -> HashMap<String, f64> {
        ...
        metrics.insert("total_orders_today".to_string(), self.total_orders_today as f64);
        metrics.insert("total_rejected_today".to_string(), self.total_rejected_today as f64);
        metrics.insert("daily_pnl".to_string(), self.daily_pnl.to_f64().unwrap_or(0.0));
