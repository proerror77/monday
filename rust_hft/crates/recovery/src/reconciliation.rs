...
use hft_core::{OrderId, Symbol, Price, Quantity, Side};
    ...
    pub fn description(&self) -> String {
        match self {
            Discrepancy::LocalOnly { local_order } => {
                format!("Order {} exists locally ({:?}) but not on exchange", 
                        local_order.order_id.0, local_order.status)
            }
            Discrepancy::ExchangeOnly { exchange_order } => {
                format!("Order {} exists on exchange ({:?}) but not locally", 
                        exchange_order.order_id.0, exchange_order.status)
            }
            Discrepancy::StatusMismatch { local_order, exchange_order } => {
                format!("Order {} status mismatch: local={:?}, exchange={:?}", 
                        local_order.order_id.0, local_order.status, exchange_order.status)
            }
            Discrepancy::QuantityMismatch { local_order, exchange_order } => {
                format!("Order {} filled quantity mismatch: local={}, exchange={}", 
...
impl Reconciler {
    ...
    pub async fn reconcile<T: ExecutionClient>(
        ...
        info!("Starting order reconciliation with {} local orders", local_orders.len());
        ...
        info!("Reconciliation completed: {} discrepancies found in {}ms", 
              discrepancies.len(), stats.reconciliation_duration_ms);
    ...
    pub async fn resolve_discrepancies<T: ExecutionClient>(
        ...
        for discrepancy in discrepancies {
            match self.resolve_single_discrepancy(discrepancy, execution_client).await {
                Ok(_) => {
    ...
    async fn fetch_exchange_orders<T: ExecutionClient>(
        ...
        let open_orders = execution_client.list_open_orders().await
    ...
    fn compare_orders(
        ...
        for order_id in all_order_ids {
            match (local_orders.get(order_id), exchange_orders.get(order_id)) {
                (Some(local), None) => {
                    // Order exists locally but not on exchange
                    if matches!(local.status, OrderStatus::New | OrderStatus::Acknowledged | OrderStatus::PartiallyFilled) {
                ...
                (Some(local), Some(exchange)) => {
                    ...
                    } else if (local.cum_qty.0 - exchange.filled_quantity.0).abs() > rust_decimal::Decimal::new(1, 8) {
