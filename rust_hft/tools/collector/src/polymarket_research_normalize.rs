...
use crate::polymarket_research_select::{
    decimal_text, json_strings, required, semantic_index, timestamp, utc_text, visit,
...
pub type ResearchNormalizationConfig = ResearchSelectionConfig;

#[derive(Debug, Clone, PartialEq, Eq, Serialize)]
pub struct ResearchNormalizationReport {
...
pub struct NormalizedResearchDataset {
    pub report: ResearchNormalizationReport,
    pub ndjson: Vec<u8>,
}
...
struct RowKey(u8, String, String, String, String, String, u64);
...
fn base(contract: &SelectedContract, surface: &str) -> Value {
    ...
    json!({
        "schema": "monday.polymarket.research_row.v1",
        "surface": surface,
    ...
    for contract in contracts.values() {
        ...
        insert(
            ...
            json!({
                ...
                "raw_market": metadata.raw_market
            }),
        );
        rows.push(row(0, contract, "", "", 0, value));
    }
    Ok(())
}
    ...
    for pending in pending {
        ...
        insert(
            ...
            json!({
                ...
                "raw_trade": pending.update["trade"]
            }),
        );
        rows.push(row(
            3,
...
fn settlement_rows(
    ...
    for pending in pending {
        ...
        if record_settlement_fingerprint(settled, market_id, &fingerprint, pending.line)? {
            ...
            let mut value = base(contract, "settlement_pair");
            insert(
                &mut value,
                json!({
                    "up_token_id": tokens[up],
                    "up_outcome": outcomes[up],
                    "up_settled_price": prices[up],
                    "down_token_id": tokens[down],
                    "down_outcome": outcomes[down],
                    "down_settled_price": prices[down],
                    "resolved_up_won": pending.update["resolved_up_won"],
                    ...
                    "source_sequence": pending.sequence,
                    "source_dataset": "crypto_expiry_reference",
                    "raw_market": pending.update["market"]
                }),
            );
            rows.push(row(
                4,
...
fn normalize_raw(
...
) -> Result<NormalizedResearchDataset> {
    ...
    Ok(NormalizedResearchDataset {
        report: ResearchNormalizationReport {
            schema: "monday.polymarket.research_normalization.v1",
            snapshot_id: format!("polymarket-btc-sol-5m-{sha256}"),
            artifact_sha256: sha256,
            artifact_bytes: u64::try_from(ndjson.len())?,
            ...
            trust_boundary: "validated staged triplets and adjacent local supersession markers; omitted remote-prefix markers are not proven absent",
...
pub fn normalize_research_segments(
    config: &ResearchNormalizationConfig,
) -> Result<NormalizedResearchDataset> {
...
mod tests {
    ...
    #[test]
    fn settlement_pair_rejects_any_changed_payload() {
        let mut settled = BTreeMap::new();
    ...
    fn canonical_rows_are_independent_of_encounter_order() {
        ...
        let (left, left_counts) = encode_rows(vec![make(1, "b"), make(0, "a")]).unwrap();
        let (right, right_counts) = encode_rows(vec![make(0, "a"), make(1, "b")]).unwrap();
