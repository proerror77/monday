...
impl ReplaySequenceValidator {
    pub fn new(market: Market, symbol: impl Into<String>) -> anyhow::Result<Self> {
    ...
    pub fn observe(
        ...
        received_at_ns: u64,
    ) -> anyhow::Result<Vec<ReplaySequenceEvent>> {
    ...
    fn apply_diff(&mut self, update: &ReplaySequenceDiff) -> anyhow::Result<bool> {
        let state = self
            .state
            .as_mut()
...
fn replay_checkpoint_seed(
    raw: &serde_json::Map<String, Value>,
    received_at_ns: u64,
) -> anyhow::Result<ReplaySequenceEvent> {
...
fn parse_replay_levels(value: Option<&Value>, field: &str) -> anyhow::Result<Vec<[String; 2]>> {
