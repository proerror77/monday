...
impl ProcessState {
    ...
    fn bump_snapshot_generation(&mut self, symbol: &str) -> u64 {
        let generation = self.snapshot_generations.entry(symbol.to_owned()).or_insert(0);
...
fn ensure_startup_spool_ready(spool_dir: &Path) -> anyhow::Result<()> {
    ...
    anyhow::ensure!(
        incomplete.is_empty(),
        "collector startup blocked by {} incomplete segment artifacts; detached recovery must isolate and drain them first",
        incomplete.len()
    );
    Ok(())
}
...
fn handle_order_book_error(
    ...
    // order book lost continuity. Keep the gap in the unsafe segment and
    // re-bootstrap this symbol without poisoning every later segment.
            ...
            let resyncs = depth_symbols_for_streams(&streams)?
                ...
                .map(|symbol| {
                    let state = states
                        ...
                        .ok_or_else(|| anyhow::anyhow!("unconfigured symbol {symbol}"))?;
                    state.verify_stream_coverage();
                    let generation = process_state.snapshot_generation(&symbol);
                    Ok(SnapshotRequest {
                        symbol,
                        generation,
                    })
...
mod tests {
    ...
    fn startup_refuses_incomplete_segments_without_mutating_them() {
        for suffix in ["jsonl.part", "jsonl.zst.tmp", "jsonl.part.corrupt"] {
            ...
            let artifact = spool_dir
                .path()
                .join(format!("part-1700000000000000000.{suffix}"));
            std::fs::write(&artifact, b"unfinished").unwrap();

            let error = ensure_startup_spool_ready(spool_dir.path()).unwrap_err();
            assert!(error.to_string().contains("detached recovery"));
