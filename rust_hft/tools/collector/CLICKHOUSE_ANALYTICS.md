...
The expected manifest SHA must come from an independently verified cache or
admission receipt. Computing it from the same untrusted manifest is not
admission evidence.
...
venue, market, symbol and time range; after a partition is complete, a retry
with the same identity and manifest SHA is a no-op, while a different manifest
SHA is a conflict.
Remote writes require `--claim-dir` on a shared filesystem used by every writer.
The materializer takes an exclusive per-partition file lock, claims the identity
as `pending`, reads the winning manifest SHA back, and only then inserts data.
If the partition is pending, a retry of the same manifest resumes with the same
deterministic row identities; a different manifest is rejected before data
insertion.
Before claiming, it reads `system.tables` and rejects a missing or incompatible
registry/data table rather than relying on an operator assertion.
...
`result_json`. Data rows also carry `row_identity` and
`materialization_version=2`. The data tables must be
`ReplacingMergeTree(materialization_version)` keyed by
`(partition_identity, row_identity)`, so resuming an identical pending manifest
cannot create a second logical row. These are schema contracts for an existing
ClickHouse database, not a provisioning script.
...
it does not create or provision tables. `CLICKHOUSE_PASSWORD` is read only when
that option is present and is never accepted as a command-line argument.
...
SHA-256 `source_revision`. Its filename must be exactly
`<manifest-sha256>.result-manifest.json`; arbitrary result-manifest paths are
rejected. The result JSON is stored as metadata, never as a replay event tape.
