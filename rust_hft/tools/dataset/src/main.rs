...
async fn main() -> Result<()> {
    ...
    match args.cmd {
        Cmd::Grid {
            ...
            run_grid(
                &venue, &symbol, start_us, end_us, step_ms, k, &ch_url, &database,
            )
            .await?
        }
        Cmd::Export {
            ...
            run_export(
                &venue, &symbol, start_us, end_us, step_ms, k, L, H_ms, tau_ticks, &ch_url,
                &database,
            )
            .await?
        }
...
impl Book {
    ...
    fn topk(&self, k: usize) -> (Vec<f64>, Vec<f64>, Vec<f64>, Vec<f64>) {
        ...
        let mut apx = Vec::new();
        let mut aqt = Vec::new();
        for (p, q) in self.bids.iter().rev().take(k) {
            bpx.push(p.0);
            bqt.push(*q);
        }
        for (p, q) in self.asks.iter().take(k) {
            apx.push(p.0);
            aqt.push(*q);
        }
        (bpx, bqt, apx, aqt)
...
async fn run_grid(
    ...
    let start_ms = start_us / 1000;
    let end_ms = end_us / 1000;
...
async fn run_export(
    ...
    while let Some(rr) = cur.next().await? {
        ...
        best_mid.push(mid);
        bids_px.push(truncate_pad(&bpx, k));
        bids_qty.push(truncate_pad(&bqt, k));
    ...
    while i + L + (h_steps as usize) < ts.len() {
        ...
        let label = if midH > mid0 + tau {
            1
        } else if midH < mid0 - tau {
            -1
        } else {
            0
        };
        ...
        for t in 0..L {
            ...
            let upd = if t == 0 {
                ...
                // 粗略：若與前一格不同則 1
                let pidx = idx - 1;
                if bids_px[idx] != bids_px[pidx] || asks_px[idx] != asks_px[pidx] {
                    1
                } else {
                    0
                }
        ...
        if rows.len() >= 200 {
            flush_samples(&ch, &mut rows).await?;
        }
        i += 1;
    }
    if !rows.is_empty() {
