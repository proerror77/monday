    ...
    group.bench_function("sustained_load", |b| {
        ...
        b.iter(|| {
            ...
            if !latencies.is_empty() {
                ...
                let variance: u64 = latencies.iter()
                    .map(|&x| {
                        let diff = if x > mean_ns { x - mean_ns } else { mean_ns - x };
                        diff * diff
                    })
...
fn bench_claude_md_requirements(c: &mut Criterion) {
    ...
    group.bench_function("all_requirements", |b| {
        b.iter(|| {
            ...
            eprintln!("  Memory Usage: {:.2}MB (target: ≤10MB)", 
            ...
            if throughput_ok && memory_ok && latency_ok {
                ...
                if !throughput_ok { eprintln!("  - Throughput below target"); }
                if !memory_ok { eprintln!("  - Memory usage above target"); }
                if !latency_ok { eprintln!("  - Latency above target"); }
            }
