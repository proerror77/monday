...
use rust_hft::utils::{
    ultra_low_latency::{
        UltraLowLatencyExecutor, LatencyMeasurer, SIMDFeatureProcessor, 
        LockFreeRingBuffer, AlignedMemoryPool, CpuAffinityManager
    },
...
impl BenchmarkData {
    fn new(size: usize) -> Self {
        ...
        Self { prices, quantities, sides, messages }
    }
}
    ...
    for &batch_size in &[1, 10, 50, 100, 500] {
        ...
        group.bench_with_input(
            ...
            |b, &batch_size| {
                ...
                let levels: Vec<(Price, Quantity)> = data.prices.iter()
                ...
                b.iter_custom(|iters| {
                    ...
                    for _ in 0..iters {
                        ...
                        let _ = orderbook.update_bulk_simd(
                            black_box(Side::Bid),
                            black_box(&levels)
                        );
                ...
                b.iter_custom(|iters| {
                    ...
                    for _ in 0..iters {
                        ...
                        let threads: Vec<_> = (0..4).map(|thread_id| {
                            ...
                            let end_idx = std::cmp::min(start_idx + chunk_size, data.prices.len());
...
fn bench_memory_efficiency(c: &mut Criterion) {
    ...
    group.bench_function("memory_usage_estimation", |b| {
        let orderbooks: Vec<_> = (0..100).map(|i| {
        ...
        b.iter(|| {
            let total_memory: usize = orderbooks.iter()
                .map(|ob| ob.estimate_memory_usage())
                .sum();
    ...
    for &data_size in &[64, 128, 256, 512] {
        ...
        group.bench_with_input(
            ...
            |b, &data_size| {
                ...
                let lob_data: Vec<f64> = (0..data_size).map(|i| 50000.0 + i as f64 * 0.01).collect();
                
                b.iter(|| {
                    #[cfg(target_arch = "x86_64")]
                    {
                        let features = unsafe {
                            processor.extract_lob_features_simd(black_box(&lob_data))
                        };
                        black_box(features);
                    }
        ...
        group.bench_with_input(
            ...
            |b, &data_size| {
                ...
                b.iter(|| {
                    ...
                    #[cfg(not(target_arch = "x86_64"))]
                    {
                        let features = processor.calculate_basic_features_simd(
                            black_box(&prices), 
                            black_box(&volumes)
                        );
                        black_box(features);
    ...
    group.bench_function("e2e_message_processing", |b| {
        ...
        b.iter_custom(|iters| {
            ...
            for i in 0..iters as usize {
                ...
                // 1. JSON parsing
                let _parsed = black_box(
                    serde_json::from_str::<serde_json::Value>(&data.messages[msg_idx])
                );
...
fn bench_ultra_low_latency_pipeline(c: &mut Criterion) {
    ...
    group.bench_function("complete_trading_pipeline", |b| {
        ...
        let executor = UltraLowLatencyExecutor::new(0)
            .unwrap_or_else(|_| {
                ...
                use rust_hft::utils::ultra_low_latency::{SIMDFeatureProcessor, LatencyMeasurer, LockFreeRingBuffer};
                ...
                impl FallbackExecutor {
                    ...
                    fn execute_ultra_fast_inference(&mut self, lob_data: &[f64]) -> anyhow::Result<Vec<f32>> {
...
fn validate_performance_targets(c: &mut Criterion) {
    ...
    group.bench_function("validate_all_targets", |b| {
        ...
        for i in 0..500 {
            ...
            orderbook.update_level_fast(Side::Bid, bid_price, quantity).unwrap();
            orderbook.update_level_fast(Side::Ask, ask_price, quantity).unwrap();
        }
