...
use rust_hft::integrations::{
    ConnectionMode, UnifiedBitgetChannel, UnifiedBitgetConfig, UnifiedBitgetConnector,
};
...
async fn benchmark_connection_establishment() -> Result<Duration, anyhow::Error> {
    ...
    // 添加订阅
    connector
        .subscribe("BTCUSDT", UnifiedBitgetChannel::OrderBook5)
        .await?;
...
async fn benchmark_message_throughput(
    concurrent_count: usize,
) -> Result<(u64, Duration), anyhow::Error> {
    let config = create_connector_config();
    let connector = UnifiedBitgetConnector::new(config);
    ...
    connector
        ...
        .await?;
    connector
        .subscribe("BTCUSDT", UnifiedBitgetChannel::Trades)
        .await?;
...
fn bench_connection_establishment(c: &mut Criterion) {
    ...
    c.bench_function("single_connection_establishment", |b| {
        b.to_async(&rt).iter_batched(
            || (), // 没有 setup
            |_| async { black_box(benchmark_connection_establishment().await) },
...
fn bench_message_throughput(c: &mut Criterion) {
    ...
    c.bench_function("message_throughput", |b| {
        b.to_async(&rt).iter_batched(
            || 1,
            |count| async move { black_box(benchmark_message_throughput(count).await) },
            BatchSize::SmallInput,
        )
    });
}
