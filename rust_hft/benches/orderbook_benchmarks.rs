...
use rust_hft::integrations::{
    high_perf_orderbook_manager::{HighPerfLevel, HighPerfOrderBook},
    BarterOrderBookManager, BitgetMessage, ExtendedLevel, HighPerfOrderBookManager,
    MultiLevelOrderBook,
};
use serde_json::json;
...
fn bench_standard_orderbook_update(c: &mut Criterion) {
    ...
    for depth in [5, 10, 20, 50].iter() {
        group.bench_with_input(BenchmarkId::new("standard", depth), depth, |b, &depth| {
        ...
        group.bench_with_input(BenchmarkId::new("high_perf", depth), depth, |b, &depth| {
