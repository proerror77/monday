...
use rust_hft::integrations::high_perf_orderbook_manager::{
    HighPerfLevel, HighPerfOrderBook, HighPerfOrderBookManager,
};
...
fn bench_vwap_calculation(c: &mut Criterion) {
    ...
    for depths in [5, 10, 20, 50].iter() {
        ...
        group.bench_with_input(BenchmarkId::from_parameter(depths), depths, |b, &depths| {
