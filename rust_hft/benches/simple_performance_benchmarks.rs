...
fn bench_orderbook_comparison(c: &mut Criterion) {
    ...
    group.bench_function("ultra_fast_orderbook", |b| {
        ...
        b.iter(|| {
            for update in &updates {
                let bid_levels: Vec<_> =
                    update.bids.iter().map(|l| (l.price, l.quantity)).collect();
                let ask_levels: Vec<_> =
                    update.asks.iter().map(|l| (l.price, l.quantity)).collect();
    ...
    group.bench_function("orderbook_update_latency", |b| {
        ...
        b.iter_custom(|iters| {
            ...
            if len > 1000 {
                ...
                if p99.as_micros() > 25 {
                    eprintln!(
                        "⚠️  P99 latency {}μs exceeds target of 25μs",
                        p99.as_micros()
                    );
                } else {
