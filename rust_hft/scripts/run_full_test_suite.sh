...
show_welcome() {
    echo -e "${PURPLE}"
    echo "══════════════════════════════════════════════════════════════════"
    echo "🚀 Rust HFT - 20+商品 × 15分钟完整数据库压力测试套件"
    echo "══════════════════════════════════════════════════════════════════"
    echo -e "${NC}"
    echo ""
    echo "此测试将执行以下步骤："
    echo "  1️⃣  启动测试环境 (ClickHouse, Redis, Prometheus, Grafana)"
    echo "  2️⃣  编译高性能 Release 版本"
    echo "  3️⃣  运行 25个商品 × 15分钟实时数据收集"
    echo "  4️⃣  持续写入真实市场数据到 ClickHouse"
    echo "  5️⃣  实时性能监控和统计分析"
    echo "  6️⃣  生成详细报告和优化建议"
    echo ""
    echo "📊 测试规模："
    ...
    echo ""
    echo "⏱️  预计耗时: ~20-25 分钟 (包含环境准备和编译)"
    echo ""
    
    log_warn "注意事项："
    echo "  • 确保网络连接稳定"
    echo "  • 确保有至少 4GB 可用内存"
    echo "  • 确保有至少 10GB 可用磁盘空间"
    echo "  • 测试期间请勿关闭终端或中断进程"
    echo ""
    
    read -p "是否确认开始完整测试？(y/N): " -n 1 -r
    echo
    if [[ ! $REPLY =~ ^[Yy]$ ]]; then
        log_info "测试已取消"
        exit 0
...
prepare_results_directory() {
    ...
    RESULTS_DIR="./test_results/full_suite_$TIMESTAMP"
    
    mkdir -p "$RESULTS_DIR"
    log_info "结果将保存到: $RESULTS_DIR"
    
    # 记录测试开始时间
    echo "完整测试套件开始时间: $(date)" > "$RESULTS_DIR/test_timeline.log"
...
step1_start_environment() {
    log_step "步骤 1/6: 启动测试环境..."
    echo "$(date): 开始启动测试环境" >> "$RESULTS_DIR/test_timeline.log"
    
    if ./scripts/start_test_environment.sh > "$RESULTS_DIR/environment_setup.log" 2>&1; then
    ...
    echo "$(date): 测试环境启动完成" >> "$RESULTS_DIR/test_timeline.log"
}
...
step3_verify_environment() {
    log_step "步骤 3/6: 验证测试环境..."
    echo "$(date): 开始环境验证" >> "$RESULTS_DIR/test_timeline.log"
    
    # 检查ClickHouse
    log_info "验证 ClickHouse 连接..."
    ...
    if [ "$db_exists" = "hft" ]; then
        log_success "✅ HFT 数据库存在"
    else
        log_error "❌ HFT 数据库不存在"
        exit 1
    fi
    ...
    echo "$(date): 环境验证完成" >> "$RESULTS_DIR/test_timeline.log"
}
