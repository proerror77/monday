...
prepare_results_directory() {
    ...
    RESULTS_DIR="./test_results/auto_test_$TIMESTAMP"
    
    mkdir -p "$RESULTS_DIR"
    log_info "结果将保存到: $RESULTS_DIR"
    
    # 记录测试开始时间
    echo "自动测试套件开始时间: $(date)" > "$RESULTS_DIR/test_timeline.log"
...
step1_start_environment() {
    ...
    log_info "启动 Docker 服务 (ClickHouse, Redis, Prometheus, Grafana)..."
    
    if docker-compose up -d > "$RESULTS_DIR/environment_setup.log" 2>&1; then
...
step6_generate_report() {
    ...
    local report_file="$RESULTS_DIR/auto_test_report.md"
    local total_records=$(curl -s "http://localhost:8123/" -d "SELECT count() FROM hft.market_data_15min" | tr -d '\n')
    
    {
        echo "# 20+商品 × 15分钟完整数据库压力测试报告 (自动执行)"
        echo ""
        echo "**测试时间**: $(date)"
...
show_final_results() {
    ...
    echo "  • 综合报告: cat $RESULTS_DIR/auto_test_report.md"
...
cleanup() {
    if [ -n "$RESULTS_DIR" ]; then
        echo "$(date): 自动测试套件结束" >> "$RESULTS_DIR/test_timeline.log"
    fi
}
