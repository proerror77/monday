...
use hft_core::{HftResult, OrderType, Price, Quantity, Side, Symbol, TimeInForce};
use num_traits::ToPrimitive;
use ports::{
    AccountView, AggregatedBar, ExecutionEvent, MarketEvent, OrderIntent, Strategy, VenueScope,
};
        ...
        match signal {
            TrendSignal::Buy => {
                ...
                if current_position < self.config.max_position {
                    let target_quantity = (self.config.position_size)
                        .min(self.config.max_position - current_position);

                    if target_quantity > 0.001 {
                        // 最小交易量
            ...
            TrendSignal::Sell => {
                ...
                if current_position > -self.config.max_position {
                    let target_quantity = (self.config.position_size)
                        .min(current_position + self.config.max_position);
