...
use rust_hft::integrations::{
    ...
    unified_bitget_connector::BitgetMessage
};
use anyhow::Result;
...
impl RealTimePerformanceStats {
    ...
    fn record_message_received(&self, symbol: &str, latency_us: u64) {
        ...
        self.total_processing_latency_us.fetch_add(latency_us, Ordering::Relaxed);
        ...
        tokio::spawn(async move {
            ...
            let entry = stats.entry(symbol_owned).or_insert_with(SymbolStats::default);
            entry.message_count += 1;
            entry.last_update = Instant::now();
            entry.avg_latency_us = (entry.avg_latency_us * (entry.message_count - 1) as f64 + latency_us as f64) / entry.message_count as f64;
        });
    }
    
    fn record_db_write_success(&self, batch_size: usize, bytes: usize, latency_us: u64) {
        self.total_records_written.fetch_add(batch_size as u64, Ordering::Relaxed);
        ...
        self.total_db_write_latency_us.fetch_add(latency_us, Ordering::Relaxed);
    }
    ...
    fn update_queue_size(&self, size: usize) {
        ...
        while size_u64 > current_max {
            match self.max_queue_size.compare_exchange_weak(
                current_max, size_u64, Ordering::Relaxed, Ordering::Relaxed
            ) {
                Ok(_) => break,
                Err(new_max) => current_max = new_max,
            }
    ...
    async fn print_detailed_stats(&self) {
        ...
        let avg_db_latency = if self.db_write_success_count.load(Ordering::Relaxed) > 0 {
            self.total_db_write_latency_us.load(Ordering::Relaxed) as f64 / self.db_write_success_count.load(Ordering::Relaxed) as f64
        } else { 0.0 };
        
        info!("📊 === 详细性能统计 [运行时间: {:?}] ===", duration);
        info!("🔗 连接状态:");
        info!("   活跃连接: {}", self.active_connections.load(Ordering::Relaxed));
        info!("   失败连接: {}", self.failed_connections.load(Ordering::Relaxed));
        info!("   重连次数: {}", self.reconnection_count.load(Ordering::Relaxed));
        
        info!("📡 数据流性能:");
        info!("   接收消息: {} ({:.1} msg/s)", total_messages, msg_rate);
        info!("   写入记录: {} ({:.1} rec/s)", total_written, write_rate);
        info!("   写入批次: {}", self.total_batches_written.load(Ordering::Relaxed));
        info!("   数据吞吐: {:.2} MB ({:.2} MB/s)", total_bytes as f64 / (1024.0 * 1024.0), throughput_mbps);
        
        info!("⚡ 延迟性能:");
        info!("   处理延迟: {:.1}μs 平均, {}μs 最小, {}μs 最大", 
        ...
        info!("🗄️  数据库性能:");
        info!("   写入成功: {}", self.db_write_success_count.load(Ordering::Relaxed));
        info!("   写入错误: {}", self.db_write_error_count.load(Ordering::Relaxed));
        info!("   当前队列: {}", self.current_queue_size.load(Ordering::Relaxed));
        info!("   最大队列: {}", self.max_queue_size.load(Ordering::Relaxed));
        info!("   队列溢出: {}", self.queue_overflow_count.load(Ordering::Relaxed));
...
fn get_target_symbols() -> Vec<String> {
    vec![
        ...
        "EOSUSDT", "ATOMUSDT", "DOTUSDT", "AVAXUSDT", "SHIBUSDT",
        "FTMUSDT", "NEARUSDT", "ALGOUSDT", "KSMUSDT", "WAVESUSDT"
        ...
        match timeout(Duration::from_millis(1000), receiver.recv()).await {
            Ok(Some(message)) => {
                ...
                if local_message_count % 2000 == 0 {
                    ...
                    info!("📡 {} 高频接收: {} 消息 ({:.1} msg/s)", symbol, local_message_count, rate);
                }
            }
            Ok(None) => {
                warn!("📡 {} 接收器关闭", symbol);
...
async fn batch_database_writer_broadcast(
    ...
    // 最终刷新
    if !batch.is_empty() {
        if let Err(e) = flush_batch_to_clickhouse(&client, &mut batch, &config, &stats, writer_id).await {
...
async fn flush_batch_to_clickhouse(
    ...
    info!("💾 写入器 {}: {} 记录写入完成 ({}μs)", writer_id, record_count, latency_us);
...
async fn setup_clickhouse_database(
    ...
    info!("✅ ClickHouse 数据库 {}.{} 设置完成", config.database_name, config.table_name);
    Ok(())
}
...
async fn generate_optimization_suggestions(
    ...
    if message_rate > 50000.0 {
        ...
        info!("⚠️  中等: 消息处理率 {:.1} msg/s，建议优化网络或解析性能", message_rate);
    } else {
...
/// 主测试函数
pub async fn run_comprehensive_database_stress_test(
    config: ComprehensiveTestConfig
) -> Result<()> {
    ...
    let (broadcast_tx, _) = tokio::sync::broadcast::channel(config.batch_size * config.writer_count * 3);
    ...
    for i in 0..config.writer_count {
        ...
        let handle = tokio::spawn(async move {
            batch_database_writer_broadcast(writer_rx, client_clone, config_clone, stats_clone, i).await
        });
...
async fn main() -> Result<()> {
    ...
    info!("   ClickHouse: {}/{}.{}", config.clickhouse_url, config.database_name, config.table_name);
