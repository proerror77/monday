...
impl MultiAccountPortfolio {
    ...
    /// 獲取或創建賬戶組合
    pub fn get_or_create_portfolio(
        &mut self,
        account_id: &trading::types::AccountId,
    ) -> &mut trading::types::Portfolio {
        self.accounts
            .entry(account_id.clone())
            .or_insert_with(|| trading::types::Portfolio::new(account_id.clone(), 0.0))
    }
    ...
    pub fn add_account(&mut self, account_id: trading::types::AccountId, initial_cash: f64) {
        self.accounts.insert(
            account_id.clone(),
            trading::types::Portfolio::new(account_id, initial_cash),
        );
    }
    ...
    pub fn remove_account(
        &mut self,
        account_id: &trading::types::AccountId,
    ) -> Option<trading::types::Portfolio> {
    ...
    pub fn get_account_mut(
        &mut self,
        account_id: &trading::types::AccountId,
    ) -> Option<&mut trading::types::Portfolio> {
...
impl CompleteOMS {
    ...
    /// 獲取賬戶投資組合
    pub async fn get_account_portfolio(
        &self,
        account_id: &AccountId,
    ) -> Option<trading::types::Portfolio> {
        self.multi_portfolio
    ...
    pub async fn get_order_stats(&self) -> OMSStats {
        ...
        let execution_times: Vec<_> = orders
            ...
            .map(|order| order.last_fill_time.unwrap_or(order.update_time) - order.create_time)
            .collect();
    ...
    pub async fn cancel_all_orders_for_account(
        ...
        let orders_to_cancel: Vec<String> = {
            ...
            orders
                .iter()
                .filter(|(_, order)| {
                    &order.account_id == account_id && !self.is_order_finished(&order.status)
                })
            ...
            OrderSide::Sell => {
                ...
                let (should_remove, realized_pnl) =
                    if let Some(position) = portfolio.positions.get_mut(&order_state.symbol) {
                        let realized_pnl = (fill.price.to_f64().unwrap_or(0.0)
    ...
    fn update_order_state(
        ...
        if execution_report.last_executed_quantity > 0.0 {
            let fill = Fill {
                ...
                quantity: rust_decimal::Decimal::from_f64_retain(
    ...
    async fn update_order_state_from_report(
        ...
        order_state.remaining_quantity =
            order_state.original_quantity - order_state.executed_quantity;

        // 更新平均成交價格
        if additional_qty > 0.0 {
            let new_fill_value = additional_qty * report.last_executed_price;
            let total_fill_value = order_state.avg_fill_price
                * (order_state.executed_quantity - additional_qty)
                + new_fill_value;
            ...
            let fill = Fill {
                ...
                quantity: rust_decimal::Decimal::from_f64_retain(additional_qty)
                    .unwrap_or_default(),
                side: order_state.side,
                ...
                .or_insert_with(|| {
                    trading::types::Position::new(
                        ...
                        if quantity_change > 0.0 {
                            ...
                            trading::types::PositionSide::Short
                        },
                        0.0,
                        price,
                    )
    ...
    async fn archive_completed_order(&self, order_state: OrderStateMachine) -> Result<(), String> {
        ...
        let _archived_report = ExecutionReport {
            ...
            transaction_time: order_state
                .last_fill_time
                .unwrap_or(order_state.update_time),
                ...
                let mut orders_to_cancel = Vec::new();
                {
                    ...
                    for (order_id, order_state) in orders_guard.iter() {
                        ...
                        if age_ms > config_timeout as u64
                            && (order_state.status == OrderStatus::New
                                || order_state.status == OrderStatus::PartiallyFilled)
                        {
