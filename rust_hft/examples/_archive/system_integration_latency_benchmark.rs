...
impl PerformanceStats {
    ...
    fn record_feature_extraction_latency(&self, latency_us: u64) {
        self.feature_extraction_latency_total_us
            .fetch_add(latency_us, Ordering::Relaxed);
        self.feature_extraction_count
    ...
    fn get_avg_feature_latency(&self) -> f64 {
        let total = self
            .feature_extraction_latency_total_us
...
async fn test_websocket_performance(args: &Args, stats: Arc<PerformanceStats>) -> Result<()> {
    ...
    for i in 0..args.connections {
        ...
        let task = tokio::spawn(async move {
            ...
            stats_clone
                .ws_connection_time_us
                .store(connect_time, Ordering::Relaxed);
            info!("連接 {} 建立成功，耗時: {}μs", i, connect_time);

            let test_start = Instant::now();
            while test_start.elapsed().as_secs() < duration
                && stats_clone.is_running.load(Ordering::Relaxed)
            {
...
async fn test_feature_extraction_performance(
    ...
    while test_start.elapsed().as_secs() < args.duration && stats.is_running.load(Ordering::Relaxed)
    {
        match tokio::time::timeout(Duration::from_millis(100), rx.recv()).await {
            Ok(Some(message)) => {
                if let BitgetMessage::OrderBook {
                    data, timestamp, ..
                } = message
                {
                    let process_start = now_micros();
...
fn print_realtime_stats(stats: &Arc<PerformanceStats>) {
    ...
    println!(
        "  處理延遲: {:.1}μs (平均)",
        stats.get_avg_processing_latency()
    );
    println!(
        "  特徵延遲: {:.1}μs (平均)",
        stats.get_avg_feature_latency()
    );

    let (p50, p95, p99) = stats.get_latency_percentiles();
    println!(
        "  延遲分佈: P50={:.1}μs, P95={:.1}μs, P99={:.1}μs",
        p50, p95, p99
    );
}
...
async fn generate_test_report(stats: &Arc<PerformanceStats>) -> TestResults {
    ...
    // 計算系統性能評分
    let msg_rate = if duration > 0.0 {
        messages as f64 / duration
    } else {
        0.0
    };
    ...
    TestResults {
        ...
        websocket_stats: WebSocketStats {
            ...
            mbps: if duration > 0.0 {
                (bytes as f64 / duration) / 1024.0 / 1024.0
            } else {
                0.0
            },
            errors,
        ...
        processing_stats: ProcessingStats {
            ...
            processing_rate: if duration > 0.0 {
        ...
        feature_stats: FeatureStats {
            ...
            extraction_rate: if duration > 0.0 {
                features as f64 / duration
            } else {
                0.0
            },
        },
...
fn print_test_results(results: &TestResults) {
    ...
    println!(
        "  消息速率: {:.0} 條/秒",
        results.websocket_stats.messages_per_sec
    );
    println!("  數據吞吐: {:.2} MB/秒", results.websocket_stats.mbps);
    println!(
        "  錯誤率: {:.2}%",
        results.websocket_stats.error_rate * 100.0
    );
    ...
    println!(
        "  處理速率: {:.0} 次/秒",
        results.processing_stats.processing_rate
    );
    println!(
        "  平均延遲: {:.1}μs",
        results.processing_stats.avg_latency_us
    );
    ...
    println!(
        "  提取速率: {:.0} 次/秒",
        results.feature_stats.extraction_rate
    );
    ...
    println!(
        "  CPU 效率: {:.1}%",
        results.system_performance.cpu_efficiency
    );
    println!(
        "  內存效率: {:.1}%",
        results.system_performance.memory_efficiency
    );
    println!(
        "  綜合評分: {:.1}%",
        results.system_performance.overall_score
    );
