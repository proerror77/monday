...
use hft_core::{OrderId, Price, Quantity, Side, Symbol, OrderType, TimeInForce, Timestamp};
use ports::{OrderIntent, ExecutionEvent, ExecutionClient, Strategy, MarketEvent, AccountView, ConnectionHealth};
...
impl ExecutionClient for MockExecutionClient {
    ...
    async fn place_order(&mut self, intent: OrderIntent) -> Result<OrderId, hft_core::HftError> {
        ...
        println!("📤 下單: {} {} {} @ {:?}", 
                 intent.symbol.0, 
                 match intent.side { Side::Buy => "買入", Side::Sell => "賣出" },
                 intent.quantity.0,
        ...
        // 模擬部分成交
        self.simulate_fill(order_id.clone(), 
    ...
    async fn modify_order(&mut self, _order_id: &OrderId, _quantity: Option<Quantity>, _price: Option<Price>) -> Result<(), hft_core::HftError> {
        Ok(())
    }
    
    async fn execution_stream(&self) -> Result<Pin<Box<dyn futures::Stream<Item = Result<ExecutionEvent, hft_core::HftError>> + Send>>, hft_core::HftError> {
        use futures::stream;
