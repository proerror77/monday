...
use rust_hft::{
    integrations::{UnifiedBitgetConnector, UnifiedBitgetConfig, ConnectionMode},
...
impl AdvancedMetrics {
    ...
    fn record_message(&self, size: usize, latency_ns: u64) {
        ...
        loop {
            ...
            if self.min_latency_ns.compare_exchange_weak(
    ...
    fn get_percentiles(&self) -> (u64, u64, u64) {  // P50, P95, P99
        let mut samples = self.latency_samples.lock();
        if samples.is_empty() {
            return (0, 0, 0);
        }
...
async fn run_high_performance_test(
    ...
    if args.enable_cpu_affinity {
        ...
        if !core_affinity::set_for_current(core_affinity::CoreId { id: core_id }) {
            warn!("Failed to set CPU affinity for connection {} to core {}", connection_id, core_id);
    ...
    // 訂閱多個通道以增加數據量
    connector.subscribe("BTCUSDT", BitgetChannel::OrderBook).await?;
    connector.subscribe("BTCUSDT", BitgetChannel::Trades).await?;
    connector.subscribe("ETHUSDT", BitgetChannel::OrderBook).await?;
    ...
    if args.verbose {
        info!("Connection {} established in {:?}", connection_id, connect_time);
    }
...
async fn run_performance_analysis(args: &Args, metrics: &Arc<AdvancedMetrics>) {
    ...
    loop {
        ...
        info!("   消息處理: {} ({:.1}/s)", report.total_messages, msgs_per_sec);
        info!("   數據流量: {:.2} MB ({:.2} MB/s)", 
              report.total_bytes as f64 / (1024.0 * 1024.0), mb_per_sec);
        info!("   延遲統計 (ns): avg={}, min={}, max={}", 
              report.avg_latency_ns, report.min_latency_ns, report.max_latency_ns);
        info!("   延遲分布 (ns): P50={}, P95={}, P99={}", 
...
async fn main() -> Result<()> {
    ...
    info!("   • CPU親和性: {}", if args.enable_cpu_affinity { "啟用" } else { "禁用" });
    ...
    for i in 0..args.connections {
        ...
        let handle = tokio::spawn(async move {
            if let Err(e) = run_high_performance_test(args_clone, i, metrics_clone, shutdown_clone).await {
    ...
    info!("📨 總消息數: {} ({:.1}/s)", final_report.total_messages, final_msgs_per_sec);
    info!("📊 總流量: {:.2} MB ({:.2} MB/s)", 
          final_report.total_bytes as f64 / (1024.0 * 1024.0), final_mb_per_sec);
    
    info!("🚀 延遲統計 (納秒/微秒):");
    info!("   • 平均: {}ns ({:.2}μs)", final_report.avg_latency_ns, final_report.avg_latency_ns as f64 / 1000.0);
    info!("   • 最小: {}ns ({:.2}μs)", final_report.min_latency_ns, final_report.min_latency_ns as f64 / 1000.0);
    info!("   • 最大: {}ns ({:.2}μs)", final_report.max_latency_ns, final_report.max_latency_ns as f64 / 1000.0);
    info!("   • P50:  {}ns ({:.2}μs)", final_report.p50_latency_ns, final_report.p50_latency_ns as f64 / 1000.0);
    info!("   • P95:  {}ns ({:.2}μs)", final_report.p95_latency_ns, final_report.p95_latency_ns as f64 / 1000.0);
    info!("   • P99:  {}ns ({:.2}μs)", final_report.p99_latency_ns, final_report.p99_latency_ns as f64 / 1000.0);
    ...
    info!("🔗 連接信息:");
    info!("   • 並發連接: {}", args.connections);
    info!("   • CPU親和性: {}", if args.enable_cpu_affinity { "已啟用" } else { "未啟用" });
