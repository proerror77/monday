...
async fn main() -> Result<()> {
    ...
    // 訂閱市場數據
    connector.subscribe(&args.symbol, SimpleChannel::OrderBook5).await?;
    connector.subscribe(&args.symbol, SimpleChannel::Trade).await?;
    ...
    let processing_task = tokio::spawn(async move {
        while let Some(message) = message_rx.recv().await {
            ...
            match message.channel {
                SimpleChannel::OrderBook5 | SimpleChannel::OrderBook1 | SimpleChannel::OrderBook15 => {
                    ...
                    if let Ok(orderbook) = parse_orderbook(&symbol_clone, &data, timestamp) {
                            ...
                            if let Ok(features) = extractor.extract_features(&orderbook, 0, timestamp) {
                                ...
                                if signal != "HOLD" {
                                    ...
                                    if signal == "BUY" {
                                        ...
                                        if verbose {
                                            info!("🟢 BUY 信號 @ {:.2} (置信度: {:.2})", 
                                                  features.mid_price.0, features.obi_l5.abs());
                                        }
                                    } else {
                                        stats.sell_signals += 1;
                                        if verbose {
                                            info!("🔴 SELL 信號 @ {:.2} (置信度: {:.2})", 
                                                  features.mid_price.0, features.obi_l5.abs());
                                        }
                                    }
                                }
                        ...
                        {
                            let mut stats = stats_clone.lock().unwrap();
                            stats.avg_latency_us = (stats.avg_latency_us * (stats.messages - 1) as f64 + latency as f64) / stats.messages as f64;
                        }
                    }
                },
                SimpleChannel::Trade => {
    ...
    let report_task = tokio::spawn(async move {
        ...
        loop {
            ...
            println!("  交易信號: {} 個 (買: {}, 賣: {})", stats.signals, stats.buy_signals, stats.sell_signals);
            println!("  最佳價格: {:.2} / {:.2} (價差: {:.4})", 
                     stats.best_bid, stats.best_ask, stats.best_ask - stats.best_bid);
    ...
    println!("信號勝率: {:.1}%", if final_stats.signals > 0 {
        (final_stats.buy_signals + final_stats.sell_signals) as f64 / final_stats.signals as f64 * 100.0
    } else {
        0.0
    });
