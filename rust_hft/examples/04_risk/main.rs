...
use rust_hft::engine::{
    AlertSeverity, EnhancedRiskConfig as RiskConfig, EnhancedRiskManager,
    EnhancedRiskReport as RiskReport, PositionRiskLevel, RecommendationPriority,
    RiskRecommendation, RiskViolation, StopOrderType,
};
...
async fn demo_basic_position_management(
    ...
    info!(
        "   總投資組合價值: ${:.2}",
        risk_report.metrics.total_portfolio_value
    );
    info!(
        "   未實現PnL: ${:.2}",
        risk_report.metrics.total_unrealized_pnl
    );
    info!(
        "   已實現PnL: ${:.2}",
        risk_report.metrics.total_realized_pnl
    );
    info!("   淨PnL: ${:.2}", risk_report.metrics.net_pnl);
    info!(
        "   集中度風險: {:.1}%",
        risk_report.metrics.concentration_risk
    );
...
async fn demo_dynamic_stop_loss(
    ...
    risk_manager
        .set_dynamic_stop_loss("BTCUSDT", StopOrderType::StopLoss)
        .await?;
    risk_manager
        .set_dynamic_stop_loss("BTCUSDT", StopOrderType::TakeProfit)
        .await?;
    risk_manager
        .set_dynamic_stop_loss("ETHUSDT", StopOrderType::TrailingStop)
        .await?;
...
async fn demo_real_time_monitoring(
    ...
    while start_time.elapsed() < monitoring_duration {
        ...
        // 更新價格
        risk_manager
            .update_market_price("BTCUSDT", btc_price)
            .await?;
        risk_manager
            .update_market_price("ETHUSDT", eth_price)
            .await?;
        risk_manager
            .update_market_price("ADAUSDT", ada_price)
            .await?;
        ...
        if price_tick % 10 == 0 {
            ...
            info!(
                "   計算延遲: {}μs",
                risk_report.metrics.calculation_latency_us
            );
...
async fn demo_portfolio_optimization(
    ...
    info!(
        "   相關性風險: {:.1}%",
        risk_report.metrics.correlation_risk
    );
    ...
    let market_scenarios = vec![
        (
            "牛市情境",
            vec![1.15, 1.20, 1.12, 1.18, 1.25, 1.10, 1.08, 1.14],
        ),
        (
            "熊市情境",
            vec![0.85, 0.75, 0.82, 0.78, 0.70, 0.88, 0.92, 0.80],
        ),
        (
            "震盪情境",
            vec![1.05, 0.95, 1.08, 0.92, 1.03, 0.97, 1.06, 0.94],
        ),
    ];
...
async fn demo_stress_testing(
    ...
    for (i, (symbol, multiplier)) in symbols.iter().zip(crash_multipliers.iter()).enumerate() {
        ...
        risk_manager
            .update_market_price(symbol, crash_price)
            .await?;
        info!(
            "📉 {} 崩跌至 ${:.2} (-{:.1}%)",
            symbol,
    ...
    for symbol in &symbols[..4] {
        ...
        risk_manager
            .update_market_price(symbol, current_price * 0.9)
            .await?; // 額外10%滑點
    }
    ...
    info!(
        "   流動性評分: {:.2}",
        liquidity_report.metrics.liquidity_score
    );
    info!(
        "   市場衝擊風險: {:.2}%",
        liquidity_report.metrics.market_impact_risk
    );
    ...
    for (i, (symbol, multiplier)) in symbols.iter().zip(recovery_multipliers.iter()).enumerate() {
        ...
        risk_manager
            .update_market_price(symbol, recovery_price)
            .await?;
    }
    ...
    info!(
        "   剩餘回撤: {:.2}%",
        recovery_report.metrics.current_drawdown
    );
...
/// 處理風險事件
async fn handle_risk_event(event: RiskEvent, risk_manager: &std::sync::Arc<EnhancedRiskManager>) {
    match event {
        RiskEvent::PositionRiskLevelChanged {
            symbol,
            old_level,
            new_level,
            reason,
        ...
        RiskEvent::VarLimitTriggered {
            current_var,
            limit,
            confidence_level,
        } => {
            warn!(
                "🔴 VaR 限制觸發: {:.1}% 置信度 VaR ${:.2} 超過限制 ${:.2}",
        ...
        RiskEvent::ConcentrationAlert {
            symbol,
            concentration_pct,
            threshold,
        } => {
            warn!(
                "⚠️ 持倉集中度警告: {} 佔比 {:.1}% 超過閾值 {:.1}%",
                symbol, concentration_pct, threshold
            );
        }

        RiskEvent::LiquidityAlert {
            symbol,
            liquidity_score,
            threshold,
        } => {
            warn!(
                "💧 流動性警告: {} 流動性評分 {:.2} 低於閾值 {:.2}",
                symbol, liquidity_score, threshold
            );
        }

        RiskEvent::MarketStressAlert {
            stress_level,
            regime,
            recommended_action,
        } => {
            warn!(
                "🌪️ 市場壓力警告: 壓力水平 {:.2} 波動率環境 {:?}",
                stress_level, regime
            );
...
async fn generate_final_risk_report(
    ...
    info!(
        "   總投資組合價值: ${:.2}",
        final_report.metrics.total_portfolio_value
    );
    ...
    info!(
        "   未實現PnL: ${:.2}",
        final_report.metrics.total_unrealized_pnl
    );
    info!(
        "   已實現PnL: ${:.2}",
        final_report.metrics.total_realized_pnl
    );
    ...
    info!(
        "   預期損失: ${:.2}",
        final_report.metrics.expected_shortfall
    );
    ...
    info!(
        "   集中度風險: {:.1}%",
        final_report.metrics.concentration_risk
    );
    info!(
        "   相關性風險: {:.1}%",
        final_report.metrics.correlation_risk
    );
    info!("   流動性評分: {:.2}", final_report.metrics.liquidity_score);
    info!(
        "   市場衝擊風險: {:.2}%",
        final_report.metrics.market_impact_risk
    );

    info!("\n🌡️ 市場環境:");
    info!(
        "   波動率環境: {:?}",
        final_report.metrics.volatility_regime
    );
    info!(
        "   市場壓力水平: {:.2}",
        final_report.metrics.market_stress_level
    );

    info!("\n⚡ 系統性能:");
    info!(
        "   計算延遲: {}μs",
        final_report.metrics.calculation_latency_us
    );
    info!(
        "   最後計算時間: {}",
    ...
    if !final_report.recommendations.is_empty() {
        ...
        for rec in &final_report.recommendations {
            ...
            info!(
                "   {} {}: {}",
                priority_emoji, rec.description, rec.suggested_action
            );
        }
    }
