...
async fn main() -> Result<()> {
    ...
    let report_handle = tokio::spawn(async move {
        ...
        while !report_shutdown.load(Ordering::Relaxed) {
            ...
            let avg_feature_time = if inferences > 0 {
                feature_time as f64 / inferences as f64
            } else {
                0.0
            };
    ...
    let error_rate = if inferences > 0 {
        errors as f64 / inferences as f64 * 100.0
    } else {
        0.0
    };
    let avg_feature_time = if inferences > 0 {
