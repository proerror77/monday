...
use rust_hft::exchanges::{ExchangeManager, BitgetExchange};
...
impl RealDataE2EFramework {
    ...
    pub async fn new(config: DryRunConfig) -> Result<Self> {
        ...
        let monitoring = Arc::new(
            MonitoringServer::new(9095).await
        ...
        oms.add_account(account_id, config.initial_balance).await
    ...
    async fn start_market_data_subscription(&self) -> Result<()> {
        ...
        if let Some(bitget) = self.exchange_manager.get_exchange("bitget").await {
            ...
            // 订阅市场数据
            exchange.subscribe_orderbook(&self.config.test_symbol, 15).await?;
            exchange.subscribe_trades(&self.config.test_symbol).await?;
            exchange.subscribe_ticker(&self.config.test_symbol).await
                        ...
                        if let Ok(event) = receiver.try_recv() {
                            ...
                            if let Some(signal) = Self::generate_trading_signal(&event).await {
                                ...
                                if Self::check_trading_risk(&signal).await {
                                    ...
                                    // Dry-Run：模拟下单（不真实提交）
                                    if let Ok(_) = Self::simulate_order_submission(&oms, &signal, &config).await {
                                        order_count += 1;
                                        {
                                            let mut stats_guard = stats.write().await;
                                        ...
                                        info!("📝 Dry-Run 模拟下单 #{}: {:?} {} @ {:.2}", 
                                            order_count, signal.side, signal.symbol, signal.price.unwrap_or(0.0));
                                    }
                                } else {
                                    let mut stats_guard = stats.write().await;
    ...
    async fn start_metrics_collector(&self) -> tokio::task::JoinHandle<()> {
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                // 更新 Prometheus 指标
                monitoring.increment_counter("e2e_market_events_total", current_stats.market_events_received);
                monitoring.increment_counter("e2e_trading_signals_total", current_stats.trading_signals_generated);
                monitoring.increment_counter("e2e_risk_checks_passed", current_stats.risk_checks_passed);
                monitoring.increment_counter("e2e_risk_checks_failed", current_stats.risk_checks_failed);
                monitoring.increment_counter("e2e_dry_orders_total", current_stats.dry_orders_simulated);
                monitoring.record_latency("e2e_avg_latency", current_stats.avg_processing_latency_us);
                monitoring.record_latency("e2e_max_latency", current_stats.max_processing_latency_us);
            }
        })
    }
    ...
    async fn print_test_results(&self, stats: &E2ETestStats) {
        ...
        if stats.market_events_received > 0 {
            let signal_rate = (stats.trading_signals_generated as f64 / stats.market_events_received as f64) * 100.0;
            info!("信号生成率: {:.2}%", signal_rate);
        }
        
        if stats.trading_signals_generated > 0 {
            let risk_pass_rate = (stats.risk_checks_passed as f64 / stats.trading_signals_generated as f64) * 100.0;
