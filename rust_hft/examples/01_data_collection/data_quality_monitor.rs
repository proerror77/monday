...
async fn main() -> Result<()> {
                        ...
                        if let Ok(mut extractor) = feature_extractor_clone.lock() {
                            if let Ok(features) = extractor.extract_features(&orderbook, 0, timestamp) {
                                ...
                                let record = MarketDataRecord {
                                    ...
                                    best_bid: features.mid_price.0 - features.spread_bps / 2.0 / 10000.0 * features.mid_price.0,
                                    best_ask: features.mid_price.0 + features.spread_bps / 2.0 / 10000.0 * features.mid_price.0,
                                ...
                                // 更新統計
                                {
                                    ...
                                    stats.avg_spread_bps = (stats.avg_spread_bps * (stats.total_records - 1) as f64 + features.spread_bps) / stats.total_records as f64;
                                    stats.avg_depth_l5 = (stats.avg_depth_l5 * (stats.total_records - 1) as f64 + features.bid_depth_l5 + features.ask_depth_l5) / stats.total_records as f64 / 2.0;
                                    stats.avg_processing_latency_us = (stats.avg_processing_latency_us * (stats.total_records - 1) as f64 + processing_latency as f64) / stats.total_records as f64;
                                    ...
                                    // 價格區間統計
                                    let price_bucket = format!("{:.0}", features.mid_price.0 / 100.0) + "00";
    ...
    let report_task = tokio::spawn(async move {
        ...
        loop {
            ...
            println!("\n📈 數據研究實時報告:");
            println!("  數據記錄: {} 條 (交易: {} 筆)", stats.total_records, stats.total_trades);
            ...
            if !stats.data_quality_scores.is_empty() {
                let avg_quality = stats.data_quality_scores.iter().sum::<f64>() / stats.data_quality_scores.len() as f64;
                println!("  數據品質: {:.3} (過去{}條)", avg_quality, stats.data_quality_scores.len());
            }
    ...
    println!("\n⚡ 系統性能:");
    println!("  平均處理延遲: {:.1}μs", final_stats.avg_processing_latency_us);
    println!("  處理速率: {:.0} 記錄/秒", final_stats.total_records as f64 / args.duration as f64);
    
    if !final_stats.data_quality_scores.is_empty() {
        let avg_quality = final_stats.data_quality_scores.iter().sum::<f64>() / final_stats.data_quality_scores.len() as f64;
...
fn parse_enhanced_orderbook(symbol: &str, _data: &serde_json::Value, timestamp: u64) -> Result<OrderBook> {
