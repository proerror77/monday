/*! 
 * 完整交易系統架構 - Holistic Trading System (修復版本)
 * 
 ...
 * 
 * 執行方式：
 * cargo run --example trading_system_holistic_fixed -- --mode dry-run
 * cargo run --example trading_system_holistic_fixed -- --mode live
 */
...
pub struct HolisticTradingSystem {
    ...
    // 數據管理
    orderbook_history: VecDeque<OrderBook>,
...
impl HolisticTradingSystem {
    pub fn new(args: Args) -> Result<Self> {
        ...
        Ok(Self {
            ...
            orderbook_history: VecDeque::with_capacity(1000),
            ...
            stats: TradingSystemStats::default(),
            risk_manager,
            latency_tracker: LatencyTracker::new(),
        })
    }
    
    /// 啟動完整交易系統 - 修復版本使用Arc<Mutex<>>
    pub async fn run(system: Arc<Mutex<Self>>) -> Result<()> {
        let symbol = {
            let sys = system.lock().unwrap();
            info!("🎯 Starting Holistic Trading System for {}", sys.symbol);
            sys.symbol.clone()
        };
        ...
        let stats_task = tokio::spawn(async move {
            Self::statistics_reporter(stats_system).await;
        });
            ...
            match message {
                BitgetMessage::OrderBook { symbol, data, timestamp, .. } => {
                    if let Ok(mut sys) = system_clone.lock() {
                        if let Err(e) = sys.process_market_data(&symbol, &data, timestamp, process_start) {
    ...
    /// 處理市場數據的核心邏輯
    fn process_market_data(&mut self, _symbol: &str, data: &Value, timestamp: u64, process_start: u64) -> Result<()> {
        // 解析 OrderBook
        let orderbook = self.parse_bitget_orderbook(data, timestamp)?;
        
        // 更新統計
        self.stats.total_lob_updates += 1;
        
        // 添加到歷史記錄
        self.orderbook_history.push_back(orderbook.clone());
        if self.orderbook_history.len() > 1000 {
        ...
        self.stats.avg_prediction_latency_us = self.latency_tracker.avg_prediction_latency();
    ...
    fn execute_trading_decision(&mut self, signal: TradingSignal, features: &FeatureSet) -> Result<()> {
        ...
        match self.mode {
            ...
            TradingMode::Live => {
                ...
                self.stats.avg_order_latency_us = self.latency_tracker.avg_order_latency();
            }
        }
        ...
        if self.current_position != 0.0 {
            ...
            if pnl_pct < -0.02 || pnl_pct > 0.015 {
                let signal = TradingSignal {
                    ...
                    confidence: 0.9,
                    suggested_price: features.mid_price,
                    suggested_quantity: rust_decimal::Decimal::from_f64_retain(self.current_position.abs()).unwrap_or_default(),
                    timestamp: now_micros(),
                    features_timestamp: features.timestamp,
                    signal_latency_us: 0,
                };
    ...
    fn update_pnl_stats(&mut self, pnl: f64) {
        ...
        self.stats.total_pnl = self.stats.realized_pnl + self.unrealized_pnl;
    ...
    /// 統計報告任務
    async fn statistics_reporter(system: Arc<Mutex<Self>>) {
        ...
        loop {
            ...
            if let Ok(sys) = system.lock() {
                ...
                info!("LOB Updates: {}, Features: {}, Signals: {}", 
                      sys.stats.total_lob_updates, sys.stats.features_extracted, sys.stats.signals_generated);
                info!("Orders: {} placed, {} filled, {} total trades", 
                      sys.stats.orders_placed, sys.stats.orders_filled, sys.stats.total_trades);
                info!("PnL: Total {:.2}, Realized {:.2}, Unrealized {:.2}", 
                      sys.stats.total_pnl, sys.stats.realized_pnl, sys.stats.unrealized_pnl);
                info!("Latency: Prediction {:.1}μs, Order {:.1}μs", 
                      sys.stats.avg_prediction_latency_us, sys.stats.avg_order_latency_us);
                info!("=====================================");
            }
        }
    }
    ...
    fn generate_final_report(&self) {
        ...
        info!("   └─ LOB Updates Processed: {}", self.stats.total_lob_updates);
        info!("   └─ Features Extracted: {}", self.stats.features_extracted);
        info!("   └─ Trading Signals Generated: {}", self.stats.signals_generated);
        
        info!("⚡ Performance Metrics:");
        info!("   └─ Avg Prediction Latency: {:.1}μs", self.stats.avg_prediction_latency_us);
        info!("   └─ Avg Order Latency: {:.1}μs", self.stats.avg_order_latency_us);
        info!("   └─ Data Quality Score: {:.3}", self.stats.data_quality_score);
        
        info!("💰 Trading Results:");
        info!("   └─ Total Orders Placed: {}", self.stats.orders_placed);
        info!("   └─ Orders Filled: {}", self.stats.orders_filled);
        info!("   └─ Total Trades: {}", self.stats.total_trades);
        info!("   └─ Total PnL: ${:.2}", self.stats.total_pnl);
        info!("   └─ Realized PnL: ${:.2}", self.stats.realized_pnl);
        info!("   └─ Win Rate: {:.1}%", self.stats.win_rate * 100.0);
        
        info!("🛡️  Risk Management:");
        info!("   └─ Max Position Used: {:.1}%", self.stats.max_position_used * 100.0);
        info!("   └─ Risk Events: {}", self.stats.risk_events);
        info!("   └─ Stop Losses Triggered: {}", self.stats.stop_losses_triggered);
...
impl RiskManager {
    fn validate_signal(&self, signal: &Option<TradingSignal>, _current_position: f64, features: &FeatureSet) -> Option<TradingSignal> {
...
async fn main() -> Result<()> {
    ...
    // 創建系統實例並包裝在 Arc<Mutex<>> 中
    let system = Arc::new(Mutex::new(HolisticTradingSystem::new(args)?));
    
    // 啟動交易系統
    HolisticTradingSystem::run(system).await?;
