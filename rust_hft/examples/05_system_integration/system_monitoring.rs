...
use rust_hft::{
    HftAppRunner, WorkflowExecutor, WorkflowStep, StepResult, FeatureSet,
    integrations::{unified_bitget_connector::*, bitget_connector::BitgetChannel},
...
impl UnifiedMonitor {
    ...
    pub fn generate_report(&self) -> MonitoringReport {
        ...
        let mut report = MonitoringReport {
            ...
            error_rate_pct: if msg_count > 0 { (err_count as f64 / msg_count as f64) * 100.0 } else { 0.0 },
            throughput_msgs_per_sec: if uptime > 0 { msg_count as f64 / uptime as f64 } else { 0.0 },
        ...
        if let Ok(perf_metrics) = self.performance_metrics.lock() {
            if !perf_metrics.is_empty() {
                ...
                report.performance_summary.insert("avg_latency_us".to_string(), avg_latency);
                report.performance_summary.insert("avg_throughput".to_string(), avg_throughput);
                
                // 性能建議
                if avg_latency > 100.0 {
                    report.recommendations.push("High latency detected - consider enabling SIMD optimizations".to_string());
                }
                if avg_throughput < 10.0 {
                    report.recommendations.push("Low throughput - check network connectivity".to_string());
                }
            }
        }
        ...
        if let Ok(signal_metrics) = self.signal_metrics.lock() {
            if !signal_metrics.is_empty() {
                ...
                let avg_confidence = recent_signals.iter()
                    .map(|s| s.confidence)
                    .sum::<f64>() / recent_signals.len() as f64;
                
                let signal_distribution = count_signal_types(&recent_signals);
                
                report.signal_summary.insert("avg_confidence".to_string(), avg_confidence);
                report.signal_summary.insert("buy_signals".to_string(), signal_distribution.get("Buy").unwrap_or(&0.0).clone());
                report.signal_summary.insert("sell_signals".to_string(), signal_distribution.get("Sell").unwrap_or(&0.0).clone());
                
                // 信號質量建議
                if avg_confidence < 0.6 {
                    report.recommendations.push("Low signal confidence - consider model retraining".to_string());
                }
            }
        }
        ...
        if report.error_rate_pct > 5.0 {
            ...
            report.recommendations.push("High error rate detected - investigate system issues".to_string());
...
impl MonitoringWorkflowStep {
    fn new(monitor: Arc<UnifiedMonitor>, duration_secs: u64, report_interval: u64) -> Box<dyn WorkflowStep> {
        Box::new(Self {
            monitor,
            duration_secs,
            report_interval,
...
impl WorkflowStep for MonitoringWorkflowStep {
    ...
    async fn execute(&mut self) -> Result<StepResult> {
        ...
        connector.subscribe(&self.monitor.symbol, BitgetChannel::OrderBook5).await?;
        ...
        let monitoring_result = tokio::time::timeout(
            ...
            app.run_with_timeout(move |message| {
                ...
                match message {
                    UnifiedBitgetMessage::OrderBook { symbol, data, timestamp, .. } => {
                        // 處理OrderBook數據並記錄性能指標
                        if let Ok(orderbook) = parse_orderbook_simple(&symbol_clone, &data, timestamp) {
                            // 特徵提取
                            ...
                            if let Ok(mut extractor) = monitor_clone.feature_extractor.lock() {
                                ...
                                let signal_metrics = SignalMetrics {
                                    ...
                                    confidence,
                                    features_quality_score: calculate_feature_quality(&features),
                                ...
                                let health = SystemHealth {
                                    ...
                                    error_rate_pct: 0.0,
                                    uptime_seconds: monitor_clone.start_time.elapsed().as_secs(),
                                };
        ...
        match monitoring_result {
            Ok(_) => {
                ...
                info!("📊 Final Stats - Messages: {}, Errors: {}, Uptime: {}s", 
                ...
                Ok(StepResult::success(&format!("Monitoring completed: {} messages processed", final_report.total_messages))
...
fn parse_orderbook_simple(symbol: &str, _data: &serde_json::Value, timestamp: u64) -> Result<OrderBook> {
...
fn calculate_data_quality(_data: &serde_json::Value) -> f64 {
    if _data.is_object() || _data.is_array() { 0.95 } else { 0.5 }
}
...
async fn main() -> Result<()> {
    ...
    info!("🚀 Starting Unified HFT Monitoring System");
    info!("Symbol: {}, Duration: {}s", args.symbol, args.duration_seconds);
    ...
    let mut workflow = WorkflowExecutor::new()
        .add_step(MonitoringWorkflowStep::new(
            monitor.clone(),
