...
use rust_hft::{
    integrations::bitget_connector::{BitgetConnector, BitgetMessage, BitgetChannel},
    ml::FeatureExtractor,
...
impl E2EMetrics {
    ...
    fn record_feature_extraction(&self, time_us: u64, success: bool) {
        ...
        if !success {
            self.feature_extraction_errors.fetch_add(1, Ordering::Relaxed);
        }
    }
    ...
    fn record_order_request(&self, time_us: u64, success: bool) {
        ...
        self.order_execution_time_us.fetch_add(time_us, Ordering::Relaxed);
        if success {
    ...
    fn record_e2e_latency(&self, latency_us: u64) {
        self.e2e_latency_sum_us.fetch_add(latency_us, Ordering::Relaxed);
        ...
        loop {
            ...
            if self.min_e2e_latency_us.compare_exchange_weak(
        ...
        loop {
            ...
            if self.max_e2e_latency_us.compare_exchange_weak(
    ...
    fn record_trade(&self, profitable: bool, position: f64, pnl: f64) {
        ...
        self.current_position.store(position.to_bits(), Ordering::Relaxed);
    ...
    fn get_comprehensive_stats(&self) -> E2EStats {
        ...
        E2EStats {
            ...
            data_errors,
            feature_extractions,
            avg_feature_time: if feature_extractions > 0 { feature_time as f64 / feature_extractions as f64 } else { 0.0 },
            feature_errors,
            inferences,
            avg_inference_time: if inferences > 0 { inference_time as f64 / inferences as f64 } else { 0.0 },
            inference_errors,
            strategy_decisions,
            avg_strategy_time: if strategy_decisions > 0 { strategy_time as f64 / strategy_decisions as f64 } else { 0.0 },
            risk_checks,
            risk_rejections,
            avg_risk_time: if risk_checks > 0 { risk_time as f64 / risk_checks as f64 } else { 0.0 },
            order_requests,
            order_executions,
            avg_order_execution_time: if order_requests > 0 { order_execution_time as f64 / order_requests as f64 } else { 0.0 },
            order_errors,
            avg_e2e_latency: if e2e_latency_count > 0 { e2e_latency_sum as f64 / e2e_latency_count as f64 } else { 0.0 },
            min_e2e_latency,
            max_e2e_latency,
            total_trades,
            profitable_trades,
...
async fn run_e2e_trading_pipeline(
    ...
    let connector_handle = {
        ...
        tokio::spawn(async move {
            connector.connect_public(move |msg| {
...
async fn main() -> Result<()> {
    ...
    let report_handle = tokio::spawn(async move {
        ...
        while !report_shutdown.load(Ordering::Relaxed) {
            ...
            info!("   📨 消息處理: {} ({:.1}/s)", stats.messages, stats.messages as f64 / elapsed);
            info!("   🔍 特徵提取: {} ({:.1}μs)", stats.feature_extractions, stats.avg_feature_time);
            info!("   🧠 模型推理: {} ({:.1}μs)", stats.inferences, stats.avg_inference_time);
            info!("   📈 策略決策: {} ({:.1}μs)", stats.strategy_decisions, stats.avg_strategy_time);
            info!("   🛡️  風險控制: {} ({:.1}μs, 拒絕: {})", 
            ...
            info!("   ⚡ 端到端延遲: {:.1}μs (min: {}μs, max: {}μs)", 
                  stats.avg_e2e_latency, stats.min_e2e_latency, stats.max_e2e_latency);
            info!("   💰 交易統計: {}/{} (勝率: {:.1}%)", 
                  stats.profitable_trades, stats.total_trades, 
                  if stats.total_trades > 0 { stats.profitable_trades as f64 / stats.total_trades as f64 * 100.0 } else { 0.0 });
            info!("   📊 當前倉位: {:.4}, 未實現PnL: ${:.2}", 
                  stats.current_position, stats.unrealized_pnl);
            info!("   ❌ 錯誤統計: 數據({}) 特徵({}) 推理({}) 執行({})", 
    ...
    info!("   📨 消息處理: {} ({:.1}/s)", final_stats.messages, final_stats.messages as f64 / total_time);
    info!("   🔍 特徵提取: {} ({:.1}μs)", final_stats.feature_extractions, final_stats.avg_feature_time);
    info!("   🧠 模型推理: {} ({:.1}μs)", final_stats.inferences, final_stats.avg_inference_time);
    info!("   📈 策略決策: {} ({:.1}μs)", final_stats.strategy_decisions, final_stats.avg_strategy_time);
    info!("   🛡️  風險控制: {} ({:.1}μs)", final_stats.risk_checks, final_stats.avg_risk_time);
    info!("   💼 訂單執行: {}/{} ({:.1}μs)", 
          final_stats.order_executions, final_stats.order_requests, final_stats.avg_order_execution_time);
    info!("   ⚡ 端到端延遲: {:.1}μs (min: {}μs, max: {}μs)", 
          final_stats.avg_e2e_latency, final_stats.min_e2e_latency, final_stats.max_e2e_latency);
    
    // 計算各階段延遲佔比
    let total_component_time = final_stats.avg_feature_time + final_stats.avg_inference_time + 
                               final_stats.avg_strategy_time + final_stats.avg_risk_time + 
                               final_stats.avg_order_execution_time;
    
    info!("   📊 延遲分解:");
    info!("      特徵提取: {:.1}% ({:.1}μs)", 
          final_stats.avg_feature_time / total_component_time * 100.0, final_stats.avg_feature_time);
    info!("      模型推理: {:.1}% ({:.1}μs)", 
          final_stats.avg_inference_time / total_component_time * 100.0, final_stats.avg_inference_time);
    info!("      策略決策: {:.1}% ({:.1}μs)", 
          final_stats.avg_strategy_time / total_component_time * 100.0, final_stats.avg_strategy_time);
    info!("      風險控制: {:.1}% ({:.1}μs)", 
          final_stats.avg_risk_time / total_component_time * 100.0, final_stats.avg_risk_time);
    info!("      訂單執行: {:.1}% ({:.1}μs)", 
          final_stats.avg_order_execution_time / total_component_time * 100.0, final_stats.avg_order_execution_time);
    ...
    // 錯誤統計
    let total_operations = final_stats.feature_extractions + final_stats.inferences + 
                          final_stats.strategy_decisions + final_stats.risk_checks + 
                          final_stats.order_requests;
    let total_errors = final_stats.data_errors + final_stats.feature_errors + 
                      final_stats.inference_errors + final_stats.order_errors;
    ...
    info!("      風險拒絕率: {:.2}%", 
          if final_stats.risk_checks > 0 { final_stats.risk_rejections as f64 / final_stats.risk_checks as f64 * 100.0 } else { 0.0 });
