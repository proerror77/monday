    ...
    pub fn record_message_processed(&self, processing_latency_us: u64) {
        ...
        if elapsed.as_secs() > 0 {
            let current_throughput =
                self.message_count.load(Ordering::Relaxed) as f64 / elapsed.as_secs_f64();
    ...
    pub fn finalize_metrics(
        ...
        if let Ok(samples) = self.latency_samples.lock() {
            if !samples.is_empty() {
                ...
                final_metrics.avg_latency_us = final_metrics.total_processing_time_us as f64
                    / final_metrics.total_messages as f64;
        ...
        if let Ok(throughput_samples) = self.throughput_samples.lock() {
            if !throughput_samples.is_empty() {
                final_metrics.peak_throughput_msg_per_sec =
                    throughput_samples.iter().fold(0.0f64, |a, &b| a.max(b));
                final_metrics.avg_throughput_msg_per_sec =
        ...
        final_metrics.throughput_target_met =
            final_metrics.throughput_msg_per_sec >= min_throughput as f64;

        // 計算總體評級
        final_metrics.overall_grade = if final_metrics.latency_target_met
            && final_metrics.throughput_target_met
            && final_metrics.success_rate >= 0.99
        {
            "A+ 優秀".to_string()
...
async fn test_dynamic_connector_performance(
    ...
    // 添加訂閱
    dynamic_connector
        .add_subscription(symbol.to_string(), BitgetChannel::Books5)
        .await?;
    dynamic_connector
        .add_subscription(symbol.to_string(), BitgetChannel::Trade)
        .await?;
...
async fn test_multi_symbol_performance(
    ...
    let final_metrics =
        monitor.finalize_metrics(target_latency_us, min_throughput * symbols.len() as u64);
    print_performance_results(&final_metrics);
...
fn print_performance_results(metrics: &PerformanceMetrics) {
    ...
    info!(
        "   峰值吞吐量: {:.1} msg/s",
        metrics.peak_throughput_msg_per_sec
    );
    info!("");
    info!("✅ 質量指標:");
...
async fn main() -> Result<()> {
    ...
    for metrics in &all_metrics {
        ...
        if !metrics.latency_target_met {
            warn!(
                "   ⚠️  延遲目標未達成: P95 {}μs > 目標 {}μs",
                metrics.p95_latency_us, args.target_latency_us
            );
        }
        if !metrics.throughput_target_met {
            warn!(
                "   ⚠️  吞吐量目標未達成: {:.1} msg/s < 目標 {} msg/s",
                metrics.throughput_msg_per_sec, args.min_throughput
            );
        }
    }

    let overall_success = all_metrics
        .iter()
        .all(|m| m.latency_target_met && m.throughput_target_met);
