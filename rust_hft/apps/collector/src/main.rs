...
#[command(author, version, about = "原始行情收集器：直接連接交易所 WS，原樣入庫 ClickHouse", long_about = None)]
struct Args {
    /// 交易所（binance/bitget/both）
    #[arg(long, default_value = "both")]
    venue: String,
    /// 以逗號分隔的 symbols，如 BTCUSDT,ETHUSDT
    #[arg(long, default_value = "BTCUSDT,ETHUSDT")] 
    symbols: String,
    /// 訂閱的類型（以逗號分隔），binance 支援：depth@100ms,depth@500ms,trade；bitget 支援：books,books5,books15,trade
    #[arg(long, default_value = "depth@100ms,trade")]
    channels: String,
    /// Bitget 產品型別（SPOT/USDT-FUTURES/COIN-FUTURES/USDC-FUTURES）
    #[arg(long, default_value = "SPOT")]
    inst_type: String,
    /// Binance 市場（spot/usdt-m/coin-m）
    #[arg(long, default_value = "usdt-m")]
    ...
    anchor_sec: u64,
    /// 自動獲取熱門標的（優先於手動指定的 symbols）
    #[arg(long)]
    auto_top_symbols: bool,
    /// 市場類型（spot/usdt-futures/coin-futures），用於自動獲取熱門標的
    #[arg(long, default_value = "usdt-futures")]
    market: String,
    /// 每個交易所獲取的標的數量
