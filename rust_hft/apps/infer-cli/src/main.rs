...
struct SampleRow { bid_px_rel: Vec<f32>, bid_qty_log: Vec<f32>, ask_px_rel: Vec<f32>, ask_qty_log: Vec<f32>, update_flag: Vec<u8> }
...
async fn main() -> Result<()> {
    ...
    let n = 1usize; let c = 4usize; let l = args.L; let k = args.k;
    ...
    let mut cur = ch.query(q).bind(&args.symbol).bind(&args.venue).bind(args.start_us).bind(args.end_us).fetch::<SampleRow>()?;

    let mut i = 0usize;
    while let Some(r) = cur.next().await? {
        // 將 L*K 向量 reshape 成 (C,L,K)
        let mut x = Array4::<f32>::zeros((1, c, l, k));
        fill_channel(x.slice_mut(s![0,0,..,..]).as_array_mut(), &r.bid_px_rel, l, k);
        fill_channel(x.slice_mut(s![0,1,..,..]).as_array_mut(), &r.bid_qty_log, l, k);
        fill_channel(x.slice_mut(s![0,2,..,..]).as_array_mut(), &r.ask_px_rel, l, k);
        fill_channel(x.slice_mut(s![0,3,..,..]).as_array_mut(), &r.ask_qty_log, l, k);
...
fn fill_channel(mut dst: ArrayViewMut2<f32>, flat: &Vec<f32>, l: usize, k: usize) {
    ...
    for t in 0..l {
        let offset = t*k;
        for j in 0..k { dst[[t,j]] = flat[offset+j]; }
    }
}
