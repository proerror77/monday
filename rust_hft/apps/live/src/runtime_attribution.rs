...
fn portfolio_attribution(
    ...
    for strategy_id in strategy_ids {
        ...
        let mut event = event;
        attach_authoritative_account_metrics(&mut event, account, runtime_truth, observed_at)?;
        events.push(event);
    }
...
fn attach_authoritative_account_metrics(
    ...
    event.metrics.insert(
        ...
        if runtime_truth.reconciliation_complete && runtime_truth.reconciliation_healthy {
            COVERAGE_COMPLETE
        } else {
            COVERAGE_MISSING
        },
    ...
    if event.kind == AttributionKind::PortfolioSnapshot
            ...
            || reconciliation_age_us > MAX_RECONCILIATION_AGE_US)
    {
        ...
        event.reason = Some(
            "authoritative venue reconciliation is missing, unhealthy, or stale; portfolio evidence withheld from promotion"
                .to_string(),
        );
    }
    Ok(())
...
mod tests {
    ...
    #[test]
    fn portfolio_snapshot_decays_without_authoritative_reconciliation() {
        let activation = activation();
        let mut state = AttributionState::new(&activation).unwrap();
