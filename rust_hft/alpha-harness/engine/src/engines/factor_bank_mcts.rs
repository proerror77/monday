...
mod tests {
    use super::*;
    use alpha_domain::{
        CexBaselineArtifactV1, CexBaselineCartNodeV1, CexBaselineFoldV1, CexBaselineGateV1,
        CexBaselineModelKindV1, CexBaselineModelV1, CexBaselinePolicyV1, CexBaselineRangeV1,
        CexFactorBankRevisionV2, CexFactorEvaluationEvidenceV2, CexFactorScreeningAttemptV2,
        CexFactorScreeningVerdictV1, CexGpPolicyV1, CexResearchContentRefV1,
        ...
        EvaluationMetrics, EvaluationProtocolV1, EvaluationWalkForwardV1, FoldEvaluationMetrics,
        FoldPredictiveMetrics, PredictiveMetrics,
    };
    use chrono::Utc;

    fn reference(id: &str, hash: String) -> CexResearchContentRefV1 {
        CexResearchContentRefV1 {
            id: id.to_string(),
            content_sha256: hash,
        }
    ...
    fn candidate_evaluation(
        ...
        let fold_predictive = (1..=protocol.walk_forward.fold_count)
            .map(|fold_index| FoldPredictiveMetrics {
                fold_index,
                row_count: protocol.walk_forward.validation_rows,
                time_series_ic: Some(0.1),
                time_series_rank_ic: Some(0.1),
            })
            .collect::<Vec<_>>();
        let folds = (1..=protocol.walk_forward.fold_count)
            .map(|fold_index| FoldEvaluationMetrics {
                fold_index,
                row_count: protocol.walk_forward.validation_rows,
                trade_count: protocol.walk_forward.validation_rows,
                total_turnover: protocol.walk_forward.validation_rows as f64,
        ...
        CandidateEvaluation {
            ...
            metrics: EvaluationMetrics {
                ...
                row_count: protocol.walk_forward.validation_rows * protocol.walk_forward.fold_count,
                trade_count: protocol.walk_forward.validation_rows
                    * protocol.walk_forward.fold_count,
                total_turnover: (protocol.walk_forward.validation_rows
                    * protocol.walk_forward.fold_count) as f64,
                mean_net_return: 0.001,
    ...
    fn failed_evaluation(
        ...
        let config = FormulaEvaluatorConfig::for_trials(multiple_testing_trials).unwrap();
        let mut failed = candidate_evaluation(evaluator_version, &config, protocol, true);
        failed.passed = false;
        failed.failure_reasons = vec!["insufficient evidence".to_string()];
        failed.metrics.mean_net_return = 0.0;
        failed.metrics.cumulative_net_return = 0.0;
        for fold in &mut failed.metrics.folds {
            fold.mean_net_return = 0.0;
            fold.cumulative_net_return = 0.0;
        }
    ...
    #[test]
    fn target_positions_reset_history_on_new_series() {
        let rows = vec![
            ResearchRow {
                series_id: 1,
        ...
        let signals = evaluate_ast(&formula, &rows).unwrap();

        assert_eq!(signals, vec![0.0, 2.0, 0.0, 3.0]);
    }
}
