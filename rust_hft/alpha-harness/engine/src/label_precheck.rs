...
use crate::{baselines::CexSupervisedDecisionPolicyV2, evaluation::EngineContext};
...
pub fn label_space_precheck(
    ...
    let mut above = 0;
    let mut excluded = 0;
    for (row, visible) in rows.iter().zip(visible) {
        if !visible {
            continue;
        }
        if row.label_available_time > cutoff {
            excluded += 1;
            continue;
        }
        let cost = crate::baselines::decision_costs(row, &context.protocol().costs)?;
        let threshold =
    ...
    Ok(LabelSpacePrecheckV1 {
        ...
        scope: "search_visible_development_and_validation_overlapping_labels".into(),
        horizon_millis,
