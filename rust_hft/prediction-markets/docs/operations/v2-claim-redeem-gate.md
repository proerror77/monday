...
cargo tree -p ploy-connectivity
...
- The market-data lane uses `polymarket_client_sdk_v2 = 0.6.0`; the archived
  vendored V1 SDK is not in an active dependency path.
- `ploy-connectivity` is a fail-closed compatibility interface and must contain
  no Polymarket SDK, wallet signer, private key, or account-readiness implementation.
