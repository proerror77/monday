...
Prediction markets are integrated into the [Monday repository](https://github.com/proerror77/monday)
at `rust_hft/prediction-markets`. This module owns event-settlement research,
...
names are compatibility identifiers; new crates and entrypoints use functional
Monday names.
...
Live configuration examples and legacy deployment assets are archived under
`docs/archive/standalone-operations` for historical analysis. They are not
approved Monday deployment entrypoints.

## Supported development scope

The active prediction-market scope inside Monday is:
...
- prediction-market data and event contracts;
...
Any future attempt to connect a prediction-market strategy to real execution requires a separate reviewed
...
- `docs`: current module docs plus explicitly archived standalone material,
  including former deployment and infrastructure trees
...
matrices belong to migration closeout or the root prediction-market CI workflow.
...
- [Monday integration boundary](../../docs/architecture/PREDICTION_MARKETS.md)
