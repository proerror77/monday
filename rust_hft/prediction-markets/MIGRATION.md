...
On 2026-07-16 the imported workspace moved to `rust_hft/prediction-markets`
because prediction markets are a Monday market-family module. The old path is
historical provenance only and must not be recreated.
...
- Compatibility production gateway: rejects probe, submit, cancel, replace, and reconcile
- Former private-key/CLOB execution implementation: removed; canonical Monday Adapter only
- Former standalone SDK authentication probe: removed; credentialed account inspection belongs to the canonical Monday execution runtime
- Former standalone deployment, infrastructure, and production credential templates: moved to `docs/archive/standalone-operations`
- Compatibility account principal/readiness commands: fail closed and direct operators to Monday runtime
- Standard runner `full` feature: does not include legacy `live-execution`
- Former Polymarket and Predict account-operation packages: retired; canonical
  Monday execution owns account inspection and mutation
