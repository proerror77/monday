...
mod tests {
    ...
    fn validated_inputs() -> Value {
        let segment = |dataset: &str, sample: u64, versions: Value| {
            ...
            let trade_completions = if sample == 0 { json!({"market-1": {"condition_id":"condition-1","symbol":"BTCUSDT","market_window_secs":300,"trade_count":1,"trade_record_ids_sha256":format!("{:x}", Sha256::digest(b"trade-v2-1\n")),"completion_sequence":2,"retrieved_at":"2026-07-17T05:01:00Z","completeness_basis":"polymarket_data_api_exhausted_after_settlement_and_stable_polls_v1","finalization_lag_secs":60,"stable_polls_required":2}}) } else { json!({}) };
            let event_types = if is_reference { json!({"market_metadata":1,"polymarket_trade":1,"market_settlement":1,"polymarket_trade_collection_complete":1}) } else { json!({"quote":2}) };
            let events = if is_reference { 4 } else { 2 };
            json!({
                "schema":"monday.polymarket.raw.v1","venue":"polymarket","dataset":dataset,"date":"2026-07-17","hour":"05","file":format!("{dataset}.ndjson.zst"),"bytes":100,"sha256":"1".repeat(64),"events":events,"start_sequence":1,"end_sequence":events,"sequence_gaps":0,"start_recorded_at":"2026-07-17T05:00:00Z","end_recorded_at":"2026-07-17T05:01:00Z","source_file":format!("{dataset}.ndjson"),"replay_scope":if sample == 0 { "complete_reference_hour_segment" } else { "complete_full_depth_sampled_normalized_hour_segment" },"record_id_versions":versions,"event_types":event_types,"recording_policy":{"quote_sample_ms":sample,"quote_depth_levels":0,"event_scoped_quotes":true},"trade_completions":trade_completions
            })
        };
