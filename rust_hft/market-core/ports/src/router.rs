    ...
    fn route_order(
        ...
        if let Some(&client_index) = available_venues.get(&self.default_venue) {
            ...
            available_venues
                .iter()
                .next()
                .map(|(&venue, &index)| RoutingDecision {
                    target_venue: venue,
                    client_index: index,
        ...
        if let Some(&client_index) = available_venues.get(&target_venue) {
            ...
            available_venues
                ...
                .map(|&index| RoutingDecision {
                    ...
                    client_index: index,
                    reason: format!(
                        "Strategy venue unavailable, fallback to default: {}",
                        self.default_venue
                    ),
                })
        }
    ...
    pub fn build(self) -> Box<dyn ExecutionRouter> {
        match self {
            RouterConfig::SameVenue { default_venue } => {
                let venue_id = VenueId::from_str(&default_venue).unwrap_or_else(|| {
                    eprintln!(
                        "Warning: Unknown venue '{}', using BINANCE as fallback",
                        default_venue
                    );
                    VenueId::BINANCE
                });
            ...
            RouterConfig::StrategyMap {
                ...
                let default_venue_id =
                    VenueId::from_str(&default_venue).unwrap_or(VenueId::BINANCE);
            ...
            RouterConfig::RoundRobin { venues } => {
                ...
                if venue_ids.is_empty() {
                    eprintln!(
                        "Warning: No valid venues in RoundRobin config, using BINANCE as fallback"
                    );
