...
#[cfg(feature = "recovery")]
use hft_recovery::{RecoveryConfig, RecoveryManager, RecoveryStats};
...
impl RuntimeRecovery {
    ...
    pub async fn perform_startup_recovery(
        ...
        // 3) 先執行快照恢復（不做交易所對賬）
        let mut empty_clients: Vec<MockExecutionClient> = Vec::new();
        let mut stats = self
            .recovery_manager
            .perform_startup_recovery::<MockExecutionClient>(
                oms.clone(),
                portfolio.clone(),
                empty_clients.as_mut_slice(),
            )
            .await
            .map_err(Into::into)?;
        ...
        {
            use crate::system_builder::{VenueConfig, VenueType};
            for venue in &runtime.config.venues {
                if matches!(venue.venue_type, VenueType::Bitget) {
                    ...
                    if let Ok(mut client) =
                        adapter_bitget_execution::BitgetExecutionClient::new(exec_cfg)
                    {
                        // 無需連線即可使用 REST 列單（根據實作決定）；這裡直接嘗試對賬
                        if let Err(e) = self
                            .recovery_manager
                            .reconcile_with_client(oms.clone(), &mut client, &mut stats)
                            .await
                        {
                            warn!("Bitget 對賬失敗: {}", e);
                        }
        ...
        #[cfg(feature = "adapter-binance-execution")]
        {
            ...
            for venue in &runtime.config.venues {
                if matches!(venue.venue_type, VenueType::Binance) {
                    let mut client = adapter_binance_execution::BinanceExecutionClient::new();
                    if let Err(e) = self
                        .recovery_manager
                        .reconcile_with_client(oms.clone(), &mut client, &mut stats)
                        .await
        ...
        info!(
            "Recovery completed: {} orders restored, {} positions restored",
