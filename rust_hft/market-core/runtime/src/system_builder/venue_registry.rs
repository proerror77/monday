...
impl SystemBuilder {
    ...
    fn register_market_streams_for_venue(
        ...
        let filtered_instruments: Vec<InstrumentSpec> = if let Some(ref shard_config) = self.shard_config {
...
fn instrument_for_venue(symbol: Symbol, venue: VenueId) -> InstrumentSpec {
    if venue == VenueId::BINANCE_TOKENIZED_SECURITIES {
        InstrumentSpec::tokenized_security_spot(symbol, venue)
    } else {
        InstrumentSpec::crypto_spot(symbol, venue)
    }
}
