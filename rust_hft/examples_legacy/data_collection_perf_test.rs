...
impl DataCollectionStats {
    ...
    pub fn record_message(&self, msg_type: &str, processing_latency_us: u64) {
        ...
        self.total_processing_latency_us.fetch_add(processing_latency_us, Ordering::Relaxed);
        ...
        while processing_latency_us > current_max {
            match self.max_processing_latency_us.compare_exchange_weak(
                current_max, processing_latency_us, Ordering::Relaxed, Ordering::Relaxed
            ) {
                Ok(_) => break,
                Err(x) => current_max = x,
            }
    ...
    pub fn record_quality_validation_latency(&self, latency_us: u64) {
        self.quality_validation_latency_us.fetch_add(latency_us, Ordering::Relaxed);
    }
    
    pub fn record_clickhouse_write_latency(&self, latency_us: u64) {
        self.clickhouse_write_latency_us.fetch_add(latency_us, Ordering::Relaxed);
    }
    
    pub fn record_error(&self, error_type: &str) {
        match error_type {
            "parse" => { self.parse_errors.fetch_add(1, Ordering::Relaxed); },
            "validation" => { self.validation_errors.fetch_add(1, Ordering::Relaxed); },
            "database" => { self.database_errors.fetch_add(1, Ordering::Relaxed); },
            "channel_overflow" => { self.channel_overflow_errors.fetch_add(1, Ordering::Relaxed); },
            _ => {}
        };
    }
    ...
    pub fn print_final_summary(&self, test_duration_secs: u64) {
        ...
        info!("\n📦 === 消息統計 ===");
        info!("總消息數: {}", total);
        info!("已處理: {} ({:.1}%)", processed, processed as f64 / total as f64 * 100.0);
        info!("已丟棄: {} ({:.1}%)", dropped, dropped as f64 / total as f64 * 100.0);
        ...
        info!("吞吐量: {} ({:.1} 消息/秒)", throughput_grade, avg_throughput);
...
impl MockDataGenerator {
    ...
    pub fn start_generation(
        ...
        let interval_micros = 1_000_000 / rate_per_second;
        ...
        tokio::spawn(async move {
            ...
            while !self.should_stop.load(Ordering::Relaxed) {
                ...
                // 輪流生成OrderBook和Trade消息
                let (msg_type, data) = if count % 3 == 0 {
                    ("trade", generate_mock_trade_data())
                } else {
                    ("orderbook", generate_mock_orderbook_data())
                };
                
                let timestamp = now_micros();
                
                if tx.send((msg_type.to_string(), data, timestamp)).await.is_err() {
                    stats.record_error("channel_overflow");
                    break;
                }
                
                // 控制生成速率
                next_send_time += Duration::from_micros(interval_micros);
                let now = Instant::now();
...
/// 生成模擬OrderBook數據
fn generate_mock_orderbook_data() -> Value {
    serde_json::json!({
        "bids": [
            ["49999.5", "1.23"],
            ["49999.0", "2.45"],
...
/// 生成模擬Trade數據
fn generate_mock_trade_data() -> Value {
    serde_json::json!({
        "price": "50000.12",
        "volume": "1.234",
        "side": if fastrand::bool() { "buy" } else { "sell" },
        "timestamp": now_micros()
    })
}
...
impl DataProcessor {
    pub fn new(enable_quality_monitoring: bool, enable_clickhouse: bool, batch_size: usize) -> Self {
        Self {
...
async fn main() -> Result<()> {
    ...
    info!("   品質監控: {}", if args.enable_quality_monitoring { "啟用" } else { "禁用" });
    info!("   ClickHouse: {}", if args.enable_clickhouse { "啟用" } else { "禁用" });
    ...
    // 啟動數據生成器
    let generator = Arc::new(MockDataGenerator::new());
    let generation_handle = generator.clone().start_generation(tx.clone(), args.rate, stats.clone());
    ...
    let processing_handle = {
        ...
        tokio::spawn(async move {
            while let Some((msg_type, data, timestamp)) = rx.recv().await {
                processor.process_message(&msg_type, data, timestamp, &stats).await;
            }
